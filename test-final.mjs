...
try {
  ...
  const tabButtons = await page
    .locator(
      '[role="tab"], button:has-text("Dashboard"), button:has-text("Board"), button:has-text("Editor"), button:has-text("Settings"), button:has-text("Portal"), button:has-text("Workspace")',
    )
    .all();
  ...
  const sidebarButtons = await page
    .locator('aside button, nav button, [role="navigation"] button')
    .all();
    ...
    try {
      ...
      if (await btn.isVisible()) {
        ...
        await page.screenshot({
          path: `screenshots/final-06-sidebar-${i + 1}.png`,
          fullPage: true,
        });
      }
    } catch (_e) {}
  ...
  console.log('\n【测试 11】测试拖放/交互');
  const draggableElements = await page
    .locator('[draggable="true"], .draggable, .sortable-item')
    .all();
