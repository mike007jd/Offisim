...
function makeMissionDeps(): MissionServiceDeps {
  ...
  return {
    newId: () => {
      idSeq += 1;
      return `mid-${idSeq.toString().padStart(4, '0')}`;
    },
    now: () => {
      clockSeq += 1;
...
async function compileSampleIR(): Promise<LoopIR> {
  const model = fixedModel({
    structuredHints: {
      ...
      acceptance: [
        {
          id: 'a1',
          description: 'search unit tests pass',
        ...
        {
          id: 'a3',
          description: 'reviewer confirms relevance UX',
          oracle: 'review',
          required: false,
        },
        {
          id: 'a4',
          description: 'product owner signs off on launch',
          oracle: 'human',
          required: true,
        },
      ],
  ...
  const result = await softwareDevelopmentProfile.compile(
    { sourcePrompt: 'add search', context: ctx },
    model,
  );
  assert.equal(
    result.status,
    'ready',
    `sample IR must compile ready: ${JSON.stringify(result.validation.findings)}`,
  );
  return result.ir!;
}
...
await check(
  'completion acceptance items map to mission criteria with the right evaluators',
  async () => {
...
await check(
  'non-determinable items become human gates — config is DERIVED, never user-authored raw JSON',
  async () => {
    const ir = await compileSampleIR();
    const packet = buildLoopExecutionPacket(revisionFor(ir), ir, []);
    for (const c of packet.missionDraft.criteria) {
      ...
      assert.ok(
        'acceptanceId' in cfg,
        'config keys off the acceptance id, derived deterministically',
      );
...
await check('buildLoopExecutionPacket is byte-deterministic for the same revision', async () => {
  ...
  assert.deepEqual(
    p1.resolvedSkills.map((s) => s.skillId),
    ['sk-1', 'sk-2'],
  );
});

await check(
  'malformed skill binding config does not crash the packet — falls back to {}',
  async () => {
    const ir = await compileSampleIR();
    const rev = revisionFor(ir);
    const skills = [
    ...
    assert.deepEqual(
      packet.resolvedSkills[0]!.config,
      {},
      'malformed config → empty object, no throw',
    );
  },
);
...
await check(
  'the packet criteria feed the EXISTING MissionService unchanged (mission engine stays green)',
  async () => {
    const ir = await compileSampleIR();
    const packet = buildLoopExecutionPacket(revisionFor(ir), ir, []);
    ...
    assert.ok(
      persisted.some((c) => c.required === 1),
      'the required criteria survive (engine requires ≥1)',
    );
