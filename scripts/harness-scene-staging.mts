...
const req = (
  actorId: string,
  affordance: StagingRequest['affordance'],
  priority = 50,
  at = 0,
...
console.log('\n[reserve] no double-booking, order-independent');
{
  ...
  const requests = [
    req('alex', 'meeting-seat'),
    req('maya', 'meeting-seat'),
    req('kai', 'meeting-seat'),
  ];
  const fwd = reserveStaging(office, requests);
  const rev = reserveStaging(office, [...requests].reverse());
  check(
    'all 3 actors seated',
    fwd.every((s) => s.anchorId !== null),
  );
  const ids = fwd.map((s) => s.anchorId);
  check('no two actors share an anchor', new Set(ids).size === ids.length);
  check(
    'reservation is order-independent (2D == 3D staging)',
    JSON.stringify(fwd) === JSON.stringify(rev),
  );
  check(
    'placements carry world position + facing + posture',
    fwd.every(
      (s) => typeof s.x === 'number' && typeof s.facing === 'number' && s.posture === 'sitting',
    ),
  );
}
...
console.log('\n[missing] absent affordance resolves to null');
{
  ...
  check(
    'meeting-seat request → null (office has none)',
    staged.find((s) => s.actorId === 'alex')?.anchorId === null,
  );
  check(
    'workstation request → seated',
    staged.find((s) => s.actorId === 'maya')?.anchorId !== null,
  );
}
...
console.log('\n[custom] arbitrary office stages by affordance only');
{
  ...
  check(
    'no two actors share an anchor',
    new Set(staged.map((s) => s.anchorId)).size === staged.length,
  );
}
...
console.log('\n[robust] duplicate instanceId still seats both actors');
{
  ...
  const office = [
    prefab('dup', 'workstation-standard', 0, 0),
    prefab('dup', 'workstation-standard', 3, 0),
  ];
  const staged = reserveStaging(office, [req('alex', 'workstation'), req('maya', 'workstation')]);
  check(
    'both actors seated despite shared instanceId',
    staged.every((s) => s.x !== null),
  );
...
console.log('\n[rotation] world anchors transform with prefab rotation');
{
  ...
  check(
    'rot0: anchor at offset +z, facing 180',
    Math.abs((anchors0[0]?.z ?? 0) - 5.55) < 1e-9 && anchors0[0]?.facing === 180,
  );
  ...
  check(
    'rot90: offset rotates to +x, facing 270',
    Math.abs((anchors90[0]?.x ?? 0) - 5.55) < 1e-9 &&
      Math.abs((anchors90[0]?.z ?? 0) - 5) < 1e-9 &&
      anchors90[0]?.facing === 270,
  );
}
...
console.log('\n[dual] workstation-dual exposes two seats');
{
  ...
  check(
    'workstation-dual has 2 workstation anchors',
    anchors.filter((w) => w.kind === 'workstation').length === 2,
  );
  const staged = reserveStaging(
    [prefab('wd', 'workstation-dual', 0, 0)],
    [req('alex', 'workstation'), req('maya', 'workstation')],
  );
  check(
    'both actors seated at the dual desk',
    staged.every((s) => s.anchorId !== null),
  );
...
console.log('\n[priority] high-priority actor claims the scarce anchor first');
{
  ...
  check(
    'higher-priority actor (zoe) seated',
    staged.find((s) => s.actorId === 'zoe')?.anchorId !== null,
  );
  check(
    'lower-priority actor (alex) stays home',
    staged.find((s) => s.actorId === 'alex')?.anchorId === null,
  );
}
...
console.log('\n[scale] anchor offset scales with the prefab');
{
  ...
  check(
    'nearest-anchor: actor takes the closer of two free seats',
    (() => {
      const office = [
        prefab('a', 'workstation-standard', 0, 0),
        prefab('b', 'workstation-standard', 10, 0),
      ];
...
console.log('\n[performance] layered state for every beat kind');
{
  ...
  const beat = (
    kind: SceneBeat['kind'],
    activityKind: SceneBeat['activityKind'] = null,
  ): SceneBeat => ({
    id: 'b',
    kind,
    priority: 50,
  ...
  const valid = (p: CharacterPerformanceState) =>
    p.locomotion === 'idle' && (p.posture === 'sit' || p.posture === 'stand');
  check(
    'every beat kind yields a valid at-anchor state',
    KINDS.every((k) => valid(performanceForBeat(beat(k)))),
  );
  check(
    'failure → worried, intensity 2',
    performanceForBeat(beat('failure')).expression === 'worried' &&
      performanceForBeat(beat('failure')).intensity === 2,
  );
  ...
  check(
    'activity[write] → type gesture',
    performanceForBeat(beat('activity', 'write')).workGesture === 'type',
  );
  check(
    'activity[read] → read gesture',
    performanceForBeat(beat('activity', 'read')).workGesture === 'read',
  );
  check(
    'activity[shell] → inspect-terminal gesture',
    performanceForBeat(beat('activity', 'shell')).workGesture === 'inspect-terminal',
  );
  check(
    'performanceForBeat is pure (same beat → identical state)',
    // biome-ignore lint/suspicious/noSelfCompare: two independent performanceForBeat calls; intentional purity assertion
    JSON.stringify(performanceForBeat(beat('research'))) ===
      JSON.stringify(performanceForBeat(beat('research'))),
  );
}
