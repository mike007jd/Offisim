...
await check(
  ...
  async () => {
    ...
    assert.equal(
      ...
      afterRunning.current_attempt_id,
      'mission bound to that attempt',
    );
    ...
    assert.equal(
      stillVerifying?.status,
      'verifying',
      'recordEvaluation does not transition the mission',
    );
...
await check(
  'invariant 1: completing with a required criterion not PASS throws; all-PASS succeeds',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
    const created = await svc.createMission(baseInput());
...
await check(
  'illegal transition: draft→completed and a transition out of cancelled both throw',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
    const created = await svc.createMission(baseInput());
...
await check(
  'repair path: verifying→repairing(prevAttempt)→running; event carries prev attempt ref',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
    const created = await svc.createMission(baseInput());
    ...
    const data = JSON.parse(repairEvent!.data_json) as {
    ...
    assert.notEqual(
      repairRun.current_attempt_id,
      prevAttemptId,
      'new attempt id for the repair run',
    );
    const attempts = await repos.missionAttempts.listByMission(created.mission_id);
...
await check(
  'recordEvaluation while NOT verifying (running) throws invariant_violation (§19)',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
    const created = await svc.createMission(baseInput());
    ...
    assert.equal(
      c0?.status,
      'pending',
      'rejected recordEvaluation did not mutate criterion status',
    );
...
await check('transition MAP rejects an illegal edge with required criteria all PASS', async () => {
  ...
  const created = await svc.createMission(
    baseInput({
      criteria: [
        { description: 'required gate', evaluatorId: 'command_exit_zero', required: true },
      ],
    }),
  );
  await svc.markReady(created.mission_id);
...
await check(
  'A1: createMission with zero required criteria is rejected (fail-fast §18.1)',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
...
await check(
  'A1: completeMission guards zero required even if a row exists (defense-in-depth)',
  async () => {
...
await check(
  'A3: completeMission closes the current attempt (status completed + finished_at)',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
    const created = await svc.createMission(baseInput());
    ...
    assert.equal(
      ...
      'completed',
      'A3: completed mission closes its attempt to completed',
    );
...
await check(
  'A3: toFailed / toBlocked / cancel each close the current attempt with the matching terminal status',
  async () => {
    // toFailed → 'failed'
    {
      const repos = freshRepos();
...
await check(
  'A3: toRepairing supersedes the failed attempt (status superseded + finished_at)',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
    const created = await svc.createMission(baseInput());
    ...
    assert.equal(
      superseded?.status,
      'superseded',
      "toRepairing closes the old attempt to 'superseded'",
    );
...
await check(
  'A4: a concurrent cancel is NOT overwritten by a stale verifying transition',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
    const created = await svc.createMission(baseInput());
    ...
    assert.equal(
      final?.status,
      'cancelled',
      'the cancel was NOT overwritten by the stale verifying write',
    );
  },
);

await check(
  'A4: transition CAS throws even for a map-LEGAL edge when the row moved underfoot',
  async () => {
    ...
    assert.equal(
      final?.status,
      'repairing',
      'the live repairing state was NOT clobbered by a stale completed',
    );
  },
);
...
await check(
  'setRootRunId stamps root_run_id on the open attempt (was null), idempotent overwrite',
  async () => {
    const repos = freshRepos();
    const svc = createMissionService(repos, makeDeps());
    const created = await svc.createMission(baseInput());
    ...
    assert.equal(
      after?.root_run_id,
      attemptId,
      'setRootRunId records the agent run id on the attempt',
    );
    ...
    assert.equal(
      after?.finished_at,
      before?.finished_at,
      'setRootRunId leaves finished_at untouched',
    );
