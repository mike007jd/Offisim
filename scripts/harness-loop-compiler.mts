...
await check('rough software request → ready IR with feedback + retry + exit states + budget', async () => {
  const model = fixedModel({
    ...
    structuredHints: {
      ...
      acceptance: [
        { id: 'a1', description: 'toggle persists across reload', oracle: 'deterministic', evaluatorId: 'command_exit_zero', required: true },
        { id: 'a2', description: 'visual review of both themes', oracle: 'review', required: false },
      ],
    },
  });
  ...
  assert.equal(result.status, 'ready', `expected ready, got ${result.status}: ${JSON.stringify(result.validation.findings)}`);
  const ir = result.ir!;
  ...
  assert.ok(ir.edges.some((e) => e.kind === 'feedback'), 'IR has a feedback edge');
  ...
  assert.ok(ir.nodes.some((n) => n.kind === 'finish'), 'IR has a finish node');
  ...
  assert.equal(ir.metadata.profileVersion, '2.2.0', 'profile version derived from bundle VERSION');
  assert.ok(softwareDevelopmentProfile.referenceAssets.every((a) => /^[0-9a-f]{64}$/.test(a.sha256)), 'every asset has a sha256');
});
...
await check('needs_input → ≤3 questions, each with a recommended default', async () => {
  ...
  assert.ok(result.questions.length >= 1 && result.questions.length <= 3, `questions in [1,3], got ${result.questions.length}`);
  for (const q of result.questions) {
    assert.ok(q.recommendedDefault.length > 0, `question ${q.id} has a recommended default`);
  }
  assert.ok(result.questions.some((q) => q.id === 'acceptance'), 'the acceptance demo is asked');
});
...
await check('tier is never asked — it defaults silently (answer > model > standard)', async () => {
  ...
  const model = fixedModel({ structuredHints: { tier: 'turbo', outcome: 'Refactor the auth module to use sessions' } });
  const result = await softwareDevelopmentProfile.compile(
    { sourcePrompt: 'refactor auth', context: baseContext() },
    model,
  );
  assert.equal(result.status, 'ready', `illegal tier must not force a question, got ${result.status}: ${JSON.stringify(result.validation.findings)}`);
  assert.equal(result.ir!.budget!.tier, 'standard', 'illegal model tier falls back to standard');
});

await check('answering the acceptance question → recompiles to ready, answered tier wins', async () => {
  ...
  assert.equal(result.status, 'ready', `answered recompile is ready, got ${result.status}: ${JSON.stringify(result.validation.findings)}`);
...
await check('malformed structuredHints (not an object) is REPAIRED to a default IR, not a crash', async () => {
  const model = fixedModel({ structuredHints: 'totally not an object' as unknown as Record<string, unknown> });
  const result = await softwareDevelopmentProfile.compile(
  ...
  assert.ok(result.ir!.completion.acceptance.some((a) => a.required), 'default acceptance has a required item');
});
...
await check('a throwing model is caught → invalid (never crashes the compiler)', async () => {
  ...
  assert.ok(result.validation.findings.some((f) => f.code === 'model.failed'), 'records the model failure');
});
...
await check('empty / oversized source prompt is rejected deterministically', async () => {
  ...
  const empty = await softwareDevelopmentProfile.compile({ sourcePrompt: '   ', context: baseContext() }, model);
  assert.equal(empty.status, 'invalid', 'empty prompt → invalid');
  assert.ok(empty.validation.findings.some((f) => f.code === 'input.empty'));

  const huge = 'x'.repeat(64 * 1024 + 1);
  const oversized = await softwareDevelopmentProfile.compile({ sourcePrompt: huge, context: baseContext() }, model);
...
function minimalValidIR(overrides?: Partial<LoopIR>): LoopIR {
  return {
    ...
    completion: {
      outcome: 'O',
      acceptance: [{ id: 'acc', description: 'works', oracle: 'deterministic', evaluatorId: 'command_exit_zero', required: true }],
      exitStates: ['success'],
    },
    humanGates: [],
    skillBindings: [],
...
await check('validator: dangling edge (to a missing node) → INVALID', () => {
  ...
  assert.ok(v.findings.some((f) => f.code === 'edge.dangling_to'), 'reports edge.dangling_to');
});
...
await check('validator: unreachable node → INVALID', () => {
  ...
  assert.ok(v.findings.some((f) => f.code === 'graph.unreachable'), 'reports graph.unreachable');
});
...
await check('validator: unbounded retry edge → INVALID', () => {
  ...
  assert.ok(v.findings.some((f) => f.code === 'edge.unbounded_retry'), 'reports edge.unbounded_retry');
});

await check('validator: a dangling edge does NOT suppress reporting an unrelated orphan node (BOTH reported)', () => {
  const ir = minimalValidIR();
  ...
  assert.ok(v.findings.some((f) => f.code === 'graph.unreachable' && f.ref === 'orphan'), 'orphan node reported as unreachable despite the dangling edge');
  assert.ok(v.findings.some((f) => f.code === 'edge.dangling_to'), 'the dangling edge is still reported');
  // No double-report: `ghost` (a dangling `to`) must NOT also be flagged unreachable.
  assert.ok(!v.findings.some((f) => f.code === 'graph.unreachable' && f.ref === 'ghost'), 'a dangling-to target is not double-reported as unreachable');
});
...
await check('validator: completion with zero required acceptance → INVALID', () => {
  const ir = minimalValidIR();
  ir.completion.acceptance = [{ id: 'opt', description: 'optional', oracle: 'review', required: false }];
  const v = validateLoopIR(ir);
...
await check('software profile rejects forbidden language even when it rides in the source prompt', async () => {
  const model = fixedModel({ structuredHints: { tier: 'standard', outcome: 'Ship the new feature reliably' } });
  const result = await softwareDevelopmentProfile.compile(
    { sourcePrompt: 'build a worker daemon with a lease/heartbeat lifecycle database', context: baseContext() },
    model,
  );
...
await check('software profile rejects forbidden language SMUGGLED into ir.title (catch-all IR scan)', async () => {
  ...
  assert.equal(result.status, 'invalid', 'forbidden language smuggled into ir.title must be rejected');
  assert.ok(result.validation.findings.some((f) => f.code === 'profile.forbidden_infra'), 'reports profile.forbidden_infra for the smuggled title');
});

await check('software profile rejects forbidden language SMUGGLED into an acceptance description', async () => {
  ...
  const model = fixedModel({
    structuredHints: {
      ...
      outcome: 'Ship the feature with green tests',
      acceptance: [
        { id: 'a1', description: 'a lease/heartbeat scheduler keeps the workers alive', oracle: 'review', required: true },
      ],
    },
  });
  const result = await softwareDevelopmentProfile.compile(
  ...
  assert.equal(result.status, 'invalid', 'forbidden language in an acceptance description must be rejected');
  assert.ok(result.validation.findings.some((f) => f.code === 'profile.forbidden_infra'));
});
...
await check('buildLoopExecutionPacket is deterministic for the same revision', async () => {
  const model = fixedModel({
    structuredHints: {
      ...
      acceptance: [
        { id: 'a1', description: 'unit tests pass', oracle: 'deterministic', evaluatorId: 'command_exit_zero', required: true },
  ...
  const skills = [
    { bindingId: 'b1', revisionId: 'rev-1', skillId: 'sk-1', skillVersion: '1.0.0', orderIndex: 0, configJson: '{"k":1}' },
  ];
  ...
  assert.ok(criteria.some((c) => c.required), 'a required criterion survives');
});
