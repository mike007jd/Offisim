...
import { extractProtectedSpans, validateProtectedSpans } from '../apps/desktop/renderer/src/assistant/enhance/protected-spans.ts';
...
function freshSystem() {
  ...
  const missionService = createMissionService(
    ...
    { now: () => new Date().toISOString(), newId: () => `mission-${repos.snapshot().chatThreads.length}-${Math.random().toString(36).slice(2, 8)}` },
  );
...
async function seedReadyLoop(svc: LoopService) {
  const loop = await svc.createLoop({ companyId: COMPANY, title: 'Ship feature', profileId: 'software-development' });
  ...
  return { loopId: loop.loopId, revisionId: save.revision.revisionId, revisionNumber: save.revision.revisionNumber };
}
...
await check('Use/insert mutates ONLY composer UI state — no mission / chat_thread / loop_invocation row', async () => {
...
await check('Send creates loop_invocation + mission link, bound to the current message/thread/project', async () => {
...
await check('the Mission REUSES the Office thread — NO dedicated mission chat_thread is minted', async () => {
  const { repos, svc, materializerDeps } = freshSystem();
  const loop = await seedReadyLoop(svc);
  ...
  assert.equal(threadsAfter, threadsBefore, 'send mints NO new chat_thread (Office thread reused)');
  const mission = await repos.missions!.findById(result.missionId);
...
await check('the pinned revision is frozen — a LATER edit does not change the executed packet', async () => {
  ...
  const V2_MODEL = fixedModel({
    structuredHints: {
      ...
      acceptance: [
        { id: 'b1', description: 'different criterion', oracle: 'deterministic', evaluatorId: 'file_exists', required: true },
      ],
    },
  });
  const v2 = await svc.saveRevision({ loopId: loop.loopId, sourcePrompt: 'pivot the loop', context: CTX }, V2_MODEL);
...
await check('an ARCHIVED loop replays its pinned ready revision (revision is immutable)', async () => {
  ...
  const inv = await repos.loopInvocations!.findById(result.invocationId);
  assert.ok(inv && inv.mission_id, 'archived-loop replay produced a linked invocation');
});
...
await check('a DELETED revision blocks send with revision-not-found and writes nothing', async () => {
  const { svc, materializerDeps, repos } = freshSystem();
  const loop = await seedReadyLoop(svc);
...
await check('a CORRUPT (non-ready / empty IR) revision blocks send', async () => {
  ...
  const loop = await svc.createLoop({ companyId: COMPANY, title: 'WIP', profileId: 'software-development' });
  const save = await svc.saveRevision({ loopId: loop.loopId, sourcePrompt: 'vague', context: CTX }, NEEDS_INPUT_MODEL);
...
await check('a mission-create failure AFTER the invocation insert leaves NO orphan invocation', async () => {
  const { repos, svc, materializerDeps } = freshSystem();
  const loop = await seedReadyLoop(svc);
...
await check('a duplicate invocation_id insert THROWS — never a silent skip (no setMissionId corruption)', async () => {
...
await check('Enhance protected-span pipeline detects + preserves the [[loop:<id>]] chip token', async () => {
  const reference = {
    id: 'chip-1',
    loopId: 'loop-x',
  ...
  assert.ok(validateProtectedSpans(enhancedKept, spans).valid, 'enhanced text that keeps the token is valid');
  const enhancedDropped = 'Kick off the loop and then clean up the workspace and report.';
  assert.ok(!validateProtectedSpans(enhancedDropped, spans).valid, 'enhanced text that DROPS the token is INVALID');
});
...
await check('a send with no active project binds project_id=null — never a hidden default', async () => {
  const { repos, svc, materializerDeps } = freshSystem();
  const loop = await seedReadyLoop(svc);
  const result = await materializeLoopSend(materializerDeps, {
    reference: { loopId: loop.loopId, revisionId: loop.revisionId },
  ...
  const inv = await repos.loopInvocations!.findById(result.invocationId);
  assert.equal(inv!.project_id, null, 'no project → project_id is null, never a fabricated default');
});
...
await check('Office no-Loop behavior is unchanged — nothing in this path writes a loop table without a chip', async () => {
  const { repos, svc } = freshSystem();
  const loop = await seedReadyLoop(svc);
