...
function adapterModel(transport: EnhanceTransport): LoopCompileModel {
  return async (input: LoopCompileInput): Promise<LoopModelOutput> => {
    ...
    const answerSteer = input.answers
      ? Object.entries(input.answers).map(([id, a]) => `${id}: ${a}`).join('\n')
      : '';
...
await check('in-flight phases win: enhancing / compiling / saving', () => {
  assert.equal(deriveAuthoringState({ ...EMPTY_AUTHORING_MODEL, prompt: 'x', enhancing: true }), 'enhancing');
  assert.equal(deriveAuthoringState({ ...EMPTY_AUTHORING_MODEL, prompt: 'x', compiling: true }), 'compiling');
  assert.equal(deriveAuthoringState({ ...EMPTY_AUTHORING_MODEL, prompt: 'x', saving: true }), 'saving');
});
...
await check('rough prompt → compile READY (via the real model adapter) → save → ready/saved', async () => {
  const { svc } = freshSystem();
  const loop = await svc.createLoop({ companyId: COMPANY, title: 'Ship feature', profileId: PROFILE });
  ...
  assert.equal(canUseInOffice(beforeSave), false, 'a previewed-but-unsaved ready revision is NOT usable');
  ...
  const saved = await svc.saveRevision(
    { loopId: loop.loopId, sourcePrompt: 'add search and keep tests green', context: CTX, selectIfReady: true },
    adapterModel(READY_TRANSPORT),
  );
  assert.equal(saved.status, 'ready');
  assert.equal(saved.revision.revisionNumber, 1, 'first save is v1');
  const def = await svc.getLoop(loop.loopId);
  assert.equal(def.currentRevisionId, saved.revision.revisionId, 'ready save selects the revision');
...
await check('a vague prompt compiles to needs_input with ≤3 questions, savable but not usable', async () => {
  const { svc } = freshSystem();
  ...
  assert.ok(preview.questions.length >= 1 && preview.questions.length <= 3, '1..3 questions surfaced');
  for (const q of preview.questions) {
    assert.ok(typeof q.recommendedDefault === 'string', 'every question carries a recommended default');
  }
  ...
  const model: LoopAuthoringModel = { ...EMPTY_AUTHORING_MODEL, prompt: 'do something', compiled: view };
  ...
  assert.equal(canUseInOffice(savedModel), false, 'even SAVED needs_input is not usable — only ready');
});
...
await check('an invalid compile blocks Use and keeps the prompt', () => {
  ...
  assert.equal(graphStateFor('invalid', view), 'invalid', 'the graph panel shows the invalid state');
});
...
await check('graphStateFor maps authoring → LoopGraphPanel state', () => {
  const ready: CompiledRevisionView = { status: 'ready', compiledIrJson: '{}', questions: [], findings: [], sourcePrompt: 'p' };
...
await check('generated details are derived read-only from the IR (outcome/budget/oracles)', async () => {
  const { svc } = freshSystem();
  const loop = await svc.createLoop({ companyId: COMPANY, title: 'Details', profileId: PROFILE });
  const saved = await svc.saveRevision(
    { loopId: loop.loopId, sourcePrompt: 'ship it with tests', context: CTX, selectIfReady: true },
    adapterModel(READY_TRANSPORT),
  );
  ...
  for (const section of sections) {
    for (const row of section.rows) {
      assert.ok(!row.value.includes('"evaluatorId"'), 'no raw evaluator JSON in the read-only view');
...
await check('parseLoopIr rejects an empty `{}` IR (needs_input/invalid → no details)', () => {
  ...
  assert.deepEqual(buildGeneratedDetails(null), [], 'no IR → no details cards (drawer shows a hint)');
});
...
await check('the adapter maps enhance output → LoopModelOutput (enhanced text + structured hints)', async () => {
  const profile = getEnhanceProfile('loop_design');
  const request = buildEnhanceRequest({ profile: 'loop_design', text: 'rough idea', protectedSpans: [], context: { companyId: COMPANY } });
  ...
  assert.equal(mapped.enhancedPrompt, 'A clearer loop description.', 'enhanced text maps to enhancedPrompt');
  assert.equal((mapped.structuredHints as Record<string, unknown>).outcome, 'works', 'hints pass through to structuredHints');
});
...
await check('BUG-1: a new question set has a distinct remount key and drops stale answer ids', () => {
  const setA = [
  ...
  assert.notEqual(questionKey(setA), questionKey(setB), 'a different question set yields a different key');
  ...
  assert.deepEqual(Object.keys(defaultsB).sort(), ['budget', 'scope'], 'set B defaults contain only set B ids');
  for (const staleId of Object.keys(defaultsA)) {
    assert.ok(!(staleId in defaultsB), `stale id "${staleId}" must NOT appear under the new question set`);
  }
  // Same id set (e.g. a recompile that re-asks the same questions) keeps the key
  // stable — no needless remount.
  assert.equal(questionKey(setA), questionKey([...setA]), 'an identical id set keeps a stable key');
});
...
await check('BUG-2: an in-flight compile blocks a second compile even with answers', async () => {
  ...
  assert.equal(secondWithAnswers, 'blocked', 'the answers path is ALSO blocked while a compile is in flight');
