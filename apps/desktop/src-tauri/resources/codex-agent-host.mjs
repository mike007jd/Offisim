  ...
  switch (message.role) {
    case "assistant":
      return [
        ...
        ...message.toolCalls && message.toolCalls.length > 0 ? [`Tool calls: ${JSON.stringify(message.toolCalls)}`] : []
      ].join("\n");
    case "tool":
      return [
...
function buildPrompt(messages) {
  ...
  if (nonSystemMessages.length === 1 && nonSystemMessages[0]?.role === "user" && nonSystemMessages[0].content.trim()) {
    return nonSystemMessages[0].content;
  }
  const transcript = nonSystemMessages.length > 0 ? nonSystemMessages.map(formatConversationMessage).join("\n\n") : "User:\n(empty conversation)";
  return [
...
function unexpectedExitMessage(stderr, code, signal) {
  return stderr ? `Codex app-server exited unexpectedly: ${stderr}` : `Codex app-server exited unexpectedly (code=${code ?? "null"}, signal=${signal ?? "null"}).`;
}
...
function reasoningSummaryText(item) {
  ...
  const parts = [...item.summary ?? [], ...item.content ?? []].map((part) => String(part).trim()).filter(Boolean);
  return parts.join("\n\n");
}
...
async function runCodexTurn(payload) {
  ...
  let timeout = null;
  const timeoutMs = typeof request.timeoutMs === "number" && Number.isFinite(request.timeoutMs) && request.timeoutMs > 0 ? request.timeoutMs : null;
  const responsePromise = new Promise((resolve, reject) => {
    ...
    client.reader.on("line", (line) => {
      ...
      try {
        ...
        switch (message?.method) {
          ...
          case "turn/completed": {
            ...
            if (turn?.status !== "completed") {
              const errorMessage = turn?.error?.message ?? message.params?.turn?.error?.additionalDetails ?? "Codex turn did not complete successfully.";
        ...
        rejectOnce(
          error instanceof Error ? error : Object.assign(new Error(String(error ?? "Unknown Codex app-server error")), {
            code: "protocol"
          })
        );
      }
