...
export interface ChatMessage {
  ...
  attemptId?: string;
  /** Pi control lane used when this boss message was queued into an active run. */
  queueBehavior?: 'steer' | 'followUp';
  /** Durable host admission state; chat persistence is the reload recovery journal. */
  queueState?: 'pending' | 'accepted' | 'consumed' | 'failed';
  /** Rust stream cursor committed atomically with this assistant checkpoint. */
  streamCursor?: number;
