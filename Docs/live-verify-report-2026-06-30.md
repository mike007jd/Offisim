...
| User-visible navigation contains Office, Loops, Market, Personnel, Settings, and Studio; Board, Timeline and company channels live inside Office. | TRUE | `apps/desktop/renderer/src/app/nav-registry.ts`; `apps/desktop/renderer/src/surfaces/SurfaceRouter.tsx`; `apps/desktop/renderer/src/surfaces/office/board/BoardStage.tsx` | Inventory covers every nav entry and Office lens. |
| Company channels are co-located in Office while remaining isolated from project work. | TRUE | `Docs/FEATURES.md`; `apps/desktop/renderer/src/surfaces/office/rail/connect/ConnectRail.tsx` | Tests preserve distinct project/collaboration storage and runtime contracts. |
...
| INV-05 | Office company channels | Project/company groups, unread counts, new direct/group draft, Ask Team, roundtable and archive states behave. | No employees, archived thread, mentions-only, roundtable. | Screenshot, collaboration tables or UI state. | Yes | AI replies require Pi auth; no project tool execution expected. |
| INV-06 | Board meeting timeline | Real `meeting_sessions` rows appear with title and time; empty storage stays honest-empty. | No rows, cancelled/completed meeting, invalid timestamp. | Screenshot and timeline query evidence. | Yes | None. |
