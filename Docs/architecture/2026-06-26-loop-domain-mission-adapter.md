...
**not** change the Mission engine internals, the Pi runtime, or Office chat.
...
Tables (migration 0005, v5): `loop_definitions`, `loop_revisions` (immutable),
...
- The Mission engine is reused, not rewritten — the adapter is the only seam.
  ...
  Missions.
- One additive migration (0005) lands; immutable revisions mean storage grows per
  edit by design (the trade for reproducibility / diffability).
