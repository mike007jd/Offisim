...
办公室场景是默认主界面，采用 2D 俯视图。它承载部门分区、走廊动线、家具布局、工位、会议室、图书馆、机房、休息区等空间语义。

- 视觉目标：真实办公室而非游戏地图；程序美术生成（模块化纸偶系统 + PixiJS Graphics API），无重手绘依赖。不使用 pixel art（AI 生成质量差），改用 Q 版矢量几何风格。

- 交互目标：拖拽员工入座、划定部门区域、查看员工状态气泡、从空间理解工作流。支持滚轮缩放与拖拽平移。

- 反馈目标：所有关键进度通过空间中的动作与气泡可视化，而非仅靠日志面板。

- 场景结构：一整层办公室，包含以下区域：
  - **部门工作区**（开发、产品、美术等）— 按 role_slug 分组，地板颜色区分
  - **图书馆 / RAG 区** — 文档知识库的空间表达，有书架装饰和 RAG 搜索活动指示
  - **休息区** — 未分配工位的员工在此等待，有沙发/盆栽装饰
  - **会议室** — 已有实现
  - 各区域动态缩放，工位数由实际员工数决定

- 角色表现：
  - **员工 = 人** — 使用模块化纸偶系统（Q 版人物），支持发色/肤色/服装色/发型/体型自定义
  - **龙虾 = OpenClaw Agent** — 使用龙虾纸偶系统（保持 OpenClaw 品牌辨识度），仅通过 `employee.installed` 事件导入
  - 所有角色支持 14+ 动画状态（idle/walking/sitting/working/thinking/talking/resting/searching/reporting/excited/blocked/success/failed/paused）
