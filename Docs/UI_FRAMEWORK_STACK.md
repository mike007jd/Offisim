...
The product shell is engine-neutral. `DesktopAgentRuntime` is the only production engine gateway, and each task selects exactly one complete runtime engine. The current shipped implementation still assembles `DesktopPiAgentRuntime`; Codex and Claude must not appear as supported until their separate gateway, conformance, secret, Usage, and release evidence tasks are complete. Offisim owns the GUI and safe projections while each native engine retains its own sessions, compaction, and global memory.
...
| Assistant surface | assistant-ui | Use ExternalStoreRuntime-style projection over `DesktopAgentRuntime` events; do not recreate engine stream parsing, draft filtering, or routing in the UI. |
...
6. `surfaces/`: Office, Mission/Loops, Personnel, Market, Studio, Settings, plus flow-owned Lifecycle/States views. Surfaces compose grammar; they do not define new chrome.
7. `runtime/desktop-agent-runtime.ts`: the sole production engine gateway. It forwards composer turns to one selected engine and projects engine-neutral events to UI/3D state; engine-specific transport and secrets stay behind adapters.
...
- Settings expresses real capability as AI Accounts / Models / Usage / Cost. API accounts may show tokens and actual/estimated cost; subscription accounts show only provider-native usage, remaining/reset/credits. Every catalog entry requires an exact leaf model id, source URL, checkedAt, ownership, capability, and availability; Pi Agent/OAuth/auth.json/models.json remain diagnostic implementation detail rather than product identity.
