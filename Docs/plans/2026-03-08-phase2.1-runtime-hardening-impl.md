...
async *chatStream(request: LlmRequest): AsyncIterable<LlmStreamChunk> {
  try {
    ...
    // OpenAI sends usage in the last chunk with empty choices
    // The stream is already consumed above; usage comes from stream_options
    yield { done: true };
  } catch (error: unknown) {
    throw this.mapError(error);
  }
...
Note: OpenAI streaming usage extraction may need adjustment based on the SDK version. The smoke tests in Task 8 will validate this.
