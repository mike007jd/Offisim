...
| **S4** | Pi wire contract (Node emitter ↔ renderer union; reasoning-before-content shape; protocol v5) | `pnpm check:pi-wire-contract` | exit 0; all wire kinds exercised; `protocolVersion 5` |
...
| Office company channels | chat/data: S1 · live: M (Office rail) |
| Meeting timeline | `meeting_sessions` has no writer; real rows project into Board timeline (S1) |
