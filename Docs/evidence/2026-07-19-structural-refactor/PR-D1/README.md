...
- 范围：纯移动到 `git/{exec,allowlist,worktree,lease,checkpoint}.rs`；`git.rs` 仅保留 Tauri 命令入口、父级导入和 re-export。分支已 push、PR #107 已 retarget 到 A3；未 merge，release `.app` live 尚未执行。
...
- 待在当前提交重新构建精确 release `.app` 后，以临时 Git 项目和本地 bare remote 验证 status / commit / push、checkpoint 创建与回滚。
- live 证据必须记录 app SHA、精确 executable path、PID、CGWindowNumber、标题和 bounds；验收前本 PR 不标记完整交付。
