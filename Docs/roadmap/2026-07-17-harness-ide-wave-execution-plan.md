...
- **项目文件访问**只走 sandboxed Tauri 命令(`project_list_dir` /
  `project_read_file` / `project_read_file_preview`),W7 进口侧不得新开
  任意路径读取通道。
...
运维教训:①Offisim 是 Tauri 单实例,GUI 验收必须整机串行;②共享 `~/.offisim` 在多分支验收间要清库重建;③并行分支同时 bump schema 时后合者必须再抬版本。

剩余:W5 best-of-N(依赖已齐,待排期)、W6 员工记忆(先出需求包)。
