...
- 表现状态：`CharacterPerformanceState`（类型在 `packages/shared-types/src/dramaturgy/performance.ts`，实现位于 `packages/dramaturgy/src/performance.ts`）= locomotion×posture×workGesture(9)×socialGesture(4)×expression(5)×prop(5?)×intensity(3)；**无 status 字段**，指示器由旧 `CharacterAction` enum（`indicators.tsx:15`）+ workload cue 驱动。
- 关键既存事实（写 spec 时已消化）：
