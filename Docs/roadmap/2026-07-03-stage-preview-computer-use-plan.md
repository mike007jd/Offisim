...
| post-7 audit remediation | done | Full-branch audit 2026-07-03 (lead + 3 subagent passes: Rust sandbox, Pi wire, off-plan diff). Fixed: one-way Raw toggle in Csv/Doc/Sheet/Slides viewers; TextViewer virtualized wrapped-line overlap (`measureElement`); viewer-shell grid→flex (truncation banner previously consumed the 1fr row); preview header meta de-jargoned + humanized sizes + trust badge; DataTable per-column min-width for wide sheets; PdfViewer lazy page render via IntersectionObserver + `Fit`→`100%`; ImageViewer dimensions line + non-passive wheel zoom; ComputerView employee display name + timeline row spacing; workspace/computer-artifact HTML iframes no longer get `allow-scripts` (deliverables keep it); `plan_media_response` clamps every response to `MAX_PREVIEW_BINARY_BYTES` (206 + Content-Range continuation, +2 cargo tests); computer `textPreview` credential redaction on BOTH sides (bridge emit + shared-types parse, +2 harness checks, host bundle rebuilt). Known gap recorded in code: `computer.sensitive.paused` is wire-contract-only, no producer — live gate remains the write-class MCP approval flow. Verified: renderer typecheck/build, cargo 149 PASS, `pnpm validate` exit 0, release `.app` live via Computer Use (long-line log, wide CSV + Raw roundtrip, XLSX, PDF lazy pages, PNG dims, Computer tab setup/empty states) |
