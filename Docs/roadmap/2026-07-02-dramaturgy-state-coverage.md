...
dramaturgy-stress **13**, conversation-run-controller **16 scenarios**,
character-clip-map **12 960-state enumeration + 9 semantic anchors**.
...
its anchor labels (e.g. `worried folds arms`) appear in the harness source and
...
| waiting for approval | beat-composer · office-projection · scene-cue · conversation-run-controller · clip-map | `approval beat carries permission resource` · `approval priority is 100` / `approval is an interrupt` · `waiter (approval) reacts in place` · `waiter performance is worried (approval)` · `approvalCount reflects the 2 awaiting-approval members` · `approval → 'user' target + 'approval' ink` · scenario `confirm approval rejects stale answers and resolves the live answer` · anchor `worried folds arms` (wait.foldarms) |
...
| permission blocked | beat-composer · scene-cue | `failureKind 'permission' → {permission, blocked, 'permission blocked'}` · `approval beat carries permission resource` · `failureKind 'permission' → ResourceCue {permission, blocked}, glyph 'P', issue leads the bubble` |
...
| character states (wait/blocked/celebrate + totality) | character-clip-map | full 12 960-state totality + determinism enumeration · anchors: `worried folds arms` (wait.foldarms) · `blocked shakes head` (blocked.headshake) · `peak completion dances` (celebrate.dance) · `handoff picks up` · `seated rest idles` |
