...
try {
  ...
  console.log('\n步骤 4: 检查模态框内容');
  const modalContent = await page.locator('[role="dialog"], .fixed.inset-0').first().innerText().catch(() => '未找到模态框内容');
  ...
  console.log('\n步骤 6: 填写公司名称');
  const nameInput = page.locator('input[placeholder*="name" i], input[placeholder*="Name" i], input[type="text"]').first();
  ...
  console.log('\n步骤 7: 尝试创建公司');
  const createButton = page.locator('button:has-text("Create"), button:has-text("创建"), button:has-text("Submit"), button:has-text("提交")').first();
  ...
  console.log('\n步骤 11: 测试面板切换');
  const tabs = await page.locator('[role="tab"], button:has-text("Dashboard"), button:has-text("Board"), button:has-text("Editor"), button:has-text("Settings")').all();
  ...
  console.log('\n步骤 12: 测试编辑器/输入区域');
  const editorArea = page.locator('textarea, [contenteditable="true"], .editor, .ProseMirror, .monaco-editor').first();
  ...
  console.log('\n步骤 14: 测试窄屏下的按钮');
  const mobileMenuBtn = page.locator('button[aria-label*="menu" i], button:has-text("☰"), button:has-text("≡")').first();
