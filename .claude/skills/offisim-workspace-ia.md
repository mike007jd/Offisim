...
Offisim has 5 peer-level workspace surfaces. Office is the default; the other 4 are "full-page workspaces" wrapped by `FullPageWorkspaceShell`.
...
- `FullPageWorkspaceAppView` = `'sops' | 'market' | 'activity-log' | 'settings'`
...
- State types are in `apps/web/src/components/workspaces/types.ts`
