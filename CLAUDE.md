...
- **Agent SDK execution lanes 已正式开放（commits `3e99f940` `feat: add agent sdk execution lanes` + `d5e0f4c9` `Add SOP-driven dual runtime engine support`）**：核心员工 runtime 现走 4 lane —— `gateway`（默认 HTTP）、`claude-agent-sdk`、`codex-agent-sdk`、`openai-agents-sdk`。Tauri 侧统一注册在 `apps/web/src/lib/tauri-engine-adapters.ts`；trusted host 命令对应 `claude_agent_execute` / `codex_agent_execute`，sidecars 在 `apps/desktop/src-tauri/src/{claude,codex}_agent_host.rs` + `resources/codex-agent-host.mjs`；Web 不注入 fetch 仍走 SDK 默认 transport。Provider 元数据 catalog 在 `catalog/provider-source-registry/` (commit `61427aab`)。**当前交付口径**：SDK provider lane 是 text/reasoning-only，不是 Offisim tool lane；文件、shell、memory、todo、skill、MCP、builtin tool 不能靠普通 SDK provider lane 暴露或验收。需要工具证据时，走默认 Offisim harness / gateway 工具路径，或未来已验证的 tool-capable employee profile；不能把 SDK lane 文本回复当作工具执行证据。SDK adapter 收到 tool request 必须 fail closed；local file/shell routing 不能选择 external A2A 员工或普通 SDK provider lane。
...
This project is indexed by GitNexus as **Offisim** (30061 symbols, 44199 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
