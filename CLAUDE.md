...
- **禁止恢复旧 Board/Live stage 浮层**：`StageRunAxisFloats` / scene 顶部 Board+Live floating tabs 已删除；`KanbanTray` 数据/CAS/⌘J 可保留为工作流能力，但不得以旧绝对定位浮层压回 Office scene。Live/run 叙事进入 `StagePipe` + assistant-ui conversation rail 的 run-record/outputs，不再作为 scene chrome。
