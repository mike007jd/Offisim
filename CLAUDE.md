...
- `apps/web` 不再是独立产品入口；迁移期可作为旧 React renderer 源码位置存在，目标归属是 `apps/desktop/renderer`。新功能、文档和验收不要再把它当 browser/web app 交付。
- `apps/launcher` 不再保留；相关包、脚本、端口、文档和验证路径随同上 OpenSpec change 删除。
...
# 迁移期如需排查旧 renderer，可临时用 apps/web；不能作为产品验收或新架构目标。
```
...
apps/
  desktop         — Tauri 2 桌面产品；renderer 迁移目标为 apps/desktop/renderer
  platform        — Hono API 服务端
  web             — legacy React renderer staging；不再作为独立产品入口，迁移后删除
  launcher        — deprecated；随 tauri-only cleanup 删除
catalog/
...
- URL routing SSOT 目标归属是 `apps/desktop/renderer/src/lib/url-routing/`；迁移期如代码仍在 `apps/web/src/lib/url-routing/`，只按 legacy renderer path 读取。workspace 切换、primary entity、overlay、filter/search 状态先序列化到 URL，再由 `useUrlSync()` 统一写 `history.pushState/replaceState`；不要恢复旧的内部 workspace history stack。
...
- **Header / StatusBar / ChatInput 职责锁定**：Header 只剩 workspace 身份位（peer pills + view-mode + project selector + API Settings CTA + file-import）。Mode chip 在 `<ChatInput>` 的 hint row（`<SessionModeChip>`，唯一 mode 入口；StatusBar 不许再加 Proxy/Human SegmentedControl）。`<StatusBar>` 通过 `dashboardSlot` / `notificationSlot` / `gitBranchSlot` 三槽承接 Dashboard 切换 + NotificationCenter + 真实 git 分支（`useGitBranch` → `git_exec` Tauri command，web 返 null）；branch/diff/checks/commit/PR-ready 属于右侧 Git tab，不允许放进全局顶栏。
- **Tasks / Kanban / Git 职责锁定**：Activity 永显；Plan 当 `usePlanStepStore().steps.length > 0 || stage === 'planning'`；Outputs 当 `useDeliverables(activeThreadId).length > 0`。Kanban 是顶部 `taskTray` 常驻 chip，空板也渲染，展开为覆盖中心场景上半部的五列执行板；`/kanban`、`/board`、`/k` 与 chip 共用 `officeState.kanbanOpen`。Tasks tab 只保留 Board 打开入口，不再 inline 嵌板。Git 是右侧 Workspace 的 Git tab：仅展示真实本地 repo 状态、diff、local commit、PR-ready compare/unavailable，不做 push、force、假 checks 或全局 `Auto-sync`。
- **Install singularity**：`<InstallDialog>` 目标归属是 desktop renderer 的 `components/app-shell/AppGlobalDialogs.tsx`；迁移期旧路径 `apps/web/src/components/app-shell/AppGlobalDialogs.tsx` 只表示 legacy renderer location。所有入口（Market detail CTA / `useDeepLinkInstall` / 文件拖拽 sideload）都过 `useInstallFlow().startRegistryInstall|startFileImport`。新增 install 入口必须复用此 hook。
...
- **Chat attachments end-to-end**：composer 三入口（paperclip / paste / drag-drop）汇 `useChatAttachmentStaging`（sha256 dedupe / 8 MB 单 / 32 MB 总 / 后台 `parseAttachment`；send 复用 staged bytes，不重读 File）；`<ChatInput onSend>` 第二参带 `attachments?: ChatInputAttachmentPayload`，`ChatPanel.handleSend` 调 `persistStagedAttachments` 落盘 → `ChatMessage.attachments` + `RunScope.pendingAttachments`，partial 失败 `rollbackPersistedAttachments` 不留 orphan。Tauri 端 `TauriAttachmentStore`（5 binary IPC command：`attachment_write/read/list/list_all/delete`，无 base64 inflation）是桌面产品路径；`WebAttachmentStore` 只作为迁移期 renderer/local-debug 兼容，不再代表独立 web 产品。`vaultRef = attachment://<companyId>/<threadId>/<UUIDv4>`。`read_attachment` 是 builtin tool，仅 gateway lane 注入，并且执行时必须匹配当前 `companyId + RunScope.threadId`；跨 company/thread 或缺 run scope 返回 `attachment-forbidden`，不能读 store。Boot 时 `attachmentGcSweeper.run()` 走 `listAll()` 扫 orphan + emit `chat.attachment.gc.swept`；thread/project/company 硬删由 runtime 安装的 attachment cascade 先删 blob 再删 repo row，soft archive 不删。文档解析器 `@offisim/doc-engine/src/import/`（PDF/DOCX/XLSX/PPTX/text/image），`pdf.worker.min.mjs` 迁移期仍可能由旧 renderer `prebuild` 拷到 `public/`，最终要纳入 desktop renderer build。
- **narrow tier (`390x844`) 口径已被 Tauri-only 决策覆盖**：narrow 只保留为 renderer/component 级响应式不变量；不能再用 web SPA in browser 作为产品验收目标。Tauri 桌面壳保持 desktop product floor (`minWidth ≥ 1024`)；最终验收只认 release `.app`。

## Key Files

| Area | Entry point | Purpose |
|------|-------------|---------|
| Desktop renderer | target `apps/desktop/renderer/src/App.tsx`; legacy `apps/web/src/App.tsx` until migrated | Root, workspace routing, runtime init |
| View types | target `apps/desktop/renderer/src/lib/app-view-layout.ts`; legacy `apps/web/src/lib/app-view-layout.ts` until migrated | OverlayKey, OfficeViewMode 类型 |
| Workspace types | target `apps/desktop/renderer/src/components/workspaces/types.ts`; legacy `apps/web/src/components/workspaces/types.ts` until migrated | WorkspaceKey, OfficeSessionState (`selectedEmployeeId` + `selectedThreadId` 等), layout tier |
| Thread bootstrap | target `apps/desktop/renderer/src/hooks/useThreadBootstrap.ts`; legacy `apps/web/src/hooks/useThreadBootstrap.ts` until migrated | 在 active project 上 `ensureProjectHasAtLeastOneThread` + 越界自动重选 |
...
| Git branch chip | target `apps/desktop/renderer/src/runtime/useGitBranch.ts`; legacy `apps/web/src/runtime/useGitBranch.ts` until migrated | `git_exec` `rev-parse --abbrev-ref HEAD` over project workspace_root |
...
| Personnel routing | target `apps/desktop/renderer/src/lib/personnel-routing.ts`; legacy `apps/web/src/lib/personnel-routing.ts` until migrated | `routeToPersonnel(id, tab)` — single entry for cross-surface employee edit |
| URL routing | target `apps/desktop/renderer/src/lib/url-routing/`; legacy `apps/web/src/lib/url-routing/` until migrated | parser + serializer + fallback + `useUrlSync`; deep links / Back / Forward 的 canonical path |
...
- 迁移期旧 renderer Vite alias 必须与 `ui-office/package.json` exports 同步；迁移完成后同一规则转移到 `apps/desktop/renderer`。新增 subpath export 时必须同步 renderer alias，否则 dev mode 动态 import 会 404。
...
- Dev 端口以 Tauri desktop 为主；旧 web=5176 / launcher=4200 只属于迁移期删除对象，platform=4100 保留。
- 修改 `shared-types` 后必须先 `pnpm --filter @offisim/shared-types build`
- **验证命令按依赖顺序串行跑**: `shared-types -> ui-core -> core -> ui-office -> desktop renderer -> desktop`。迁移期旧 `web build` 仍会读 `core/dist`，不要并行跑 `core/ui-office/web`，否则容易拿到旧产物产生假失败/假通过。
...
- Linux/CI 在 cleanup 完成前仍可能需要 `--filter '!@offisim/desktop' --filter '!@offisim/launcher'` 跳过 Tauri；launcher 删除后同步移除该过滤。
...
- **desktop 是 Tauri v2 产品壳**: 当前 `frontendDist` 若仍指 `../../web/dist`，只表示迁移期旧 renderer 产物；最终必须指向 desktop-owned renderer dist。
...
### Legacy Renderer Provider Defaults

- 以下只描述迁移期旧 renderer debug 路径，不再是产品路线。
...
- 这条能力的目标只剩迁移期 renderer 排查，不替代 Tauri release `.app` 正式本地工作流
...
- 历史 `web` live：真实 MiniMax 请求曾跑通，底部 token / cost / latency 都是真值；该记录只作旧 renderer 能力背景，不再作为产品验收目标。
