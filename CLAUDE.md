...
## Current Architecture Decision (2026-05-18)

- 开源前结构目标：Offisim 只保留 **Tauri v2 桌面产品**。执行源是 `openspec/changes/simplify-to-tauri-only-desktop-architecture/`。
- React renderer 已归属 `apps/desktop/renderer`；仓库不再保留 standalone web 产品或 browser runtime。
- launcher 已删除；不要恢复 launcher 包或端口、脚本或验证路径。
- 最终验收只认当前 worktree 的 Tauri release `.app` + Computer Use 真实交互；localhost、dev webview、浏览器 dev server 只能用于排查，不能作为桌面或端到端验收证据。
- `offisim-core` harness 仍是默认 runtime。模型调用属于 model transport / provider adapter，不存在“普通 SDK lane”；SDK 只可作为底层 transport 细节或已验证的 SDK-native employee runtime profile。
...
pnpm --filter @offisim/desktop dev      # 启动 Tauri v2 桌面应用（当前唯一产品入口）
...
apps/
  desktop         — Tauri 2 桌面产品；React renderer 位于 apps/desktop/renderer
...
7 个 peer-level workspace, 统一走 `AppLayout`。Office 时 side panel 全挂，非 Office 时 side panel 传 null、center 走 `WorkspaceRouter`。Header 按 `activeWorkspace` 自适应。

| Workspace | Key | 描述 |
|-----------|-----|------|
| Office | `office` | 3D/2D 办公场景, AppLayout 全 slot；右栏 Project → Threads → Chat 三层 IA, ChatInput 内置 Mode chip |
| Workspace | `workspace` | 协作套件（飞书/Teams 式企业软件层）。`.suite` = 64px app-rail + split(320 list + detail)。7-app rail 已落：Messenger / Approvals / Docs / Calendar / Meetings / Contacts / Workplace。Messenger（同源 `chat_threads`，team/direct/system bot channel，复用 `ChatPanel` + 全局 `useChatSessionStore`，thread/employee 选择 clamp Office SSOT）+ Approvals OA（4 审批 kind 信箱，读 `active_thread_interactions`+`interaction_history`，To-do/Done 过滤，live pending 走 `respondToInteraction` 真实 resolve、cross-thread 项 Open-in-chat clamp `selectedThreadId`）是深绑定；Docs/Calendar/Meetings/Contacts/Workplace 已有独立 app surface，但深数据绑定仍是未完成项。 |
...
- `WorkspaceKey` = `'office' | 'sops' | 'market' | 'personnel' | 'workspace' | 'activity-log' | 'settings'`
- **Workspace suite**：`packages/ui-office/src/components/workspace-suite/`（`WorkspaceSuite`/`MessengerApp`/`ApprovalsApp`/`SystemChannel` + `useMessengerConversations`/`useApprovalsInbox`），经 `web.ts` 导出。URL = `/workspace?app=messenger|approvals|docs|calendar|meetings|contacts|workplace`（messenger 裸路径）。session state `WorkspaceSuiteSessionState`（activeApp/approvalsFilter/approvalsSelectedHistoryId）。**无新表**——Approvals 走 `repos.threads.findByCompany` fan-out per-thread `activeInteractions.findByThread`+`interactionHistory.listByThread`（无 company-wide 列举接口）。Messenger 选择**不许另起 setter**，走 `workspaceSuiteProps.onSelectThread`(=Office `handleSelectThread`)+`onSelectDirectEmployee`(=Office `handleSelectEmployee`)。
- `OverlayKey` = `'employee-creator' | 'office-editor' | 'company-select' | 'studio'`（正交于 workspace；员工 edit 不再走 overlay，统一路由 Personnel）
- URL routing SSOT 位于 `apps/desktop/renderer/src/lib/url-routing/`。workspace 切换、primary entity、overlay、filter/search 状态先序列化到 URL，再由 `useUrlSync()` 统一写 `history.pushState/replaceState`；不要恢复旧的内部 workspace history stack。
...
- **V3 shell 职责锁定（`rebuild-office-shell-v3`，2026-05-23）**：**没有全局底部 StatusBar**——`packages/ui-office/src/components/layout/StatusBar.tsx` + `EnergyMeter.tsx` 已删，`AppLayout` 去掉 `statusBar` slot（LEFT 296 / RIGHT 448），index.ts/web.ts 不再导出 StatusBar。原 StatusBar 驻留物全 diegetic 迁移：EnergyMeter(token+cost)→`SceneCostReadout`（stage 右下 absolute pill，`useDashboardMetrics`，`isRunning` 时 accent-ring + pulse；**latency 不在此**）；run-state headline + Stop→`StagePipe`（worker zone 上方 pill，step+assignee+progress，Stop=`isRunning && onAbort→abortExecution()`，**不进 composer**；abort 后 collapse muted `Stopped at #N` + 下方 Resume/Discard）；latency→`LiveRunOverlay` header；model name→composer model-chip（Phase 1，不在 shell chrome）；pending-interaction→`StagePipe` `Needs input` cue + Live overlay + 右栏 interaction bubble/HIL（Phase 1/lifecycle）。通知是 stage `SceneCostReadout` 旁的 `NotificationCenter`——26×26 圆 button + `.nb-dot` 角标 + popover（**无铃铛 header chrome、无 status-bar count 段**）；Header 不再 mount notification slot。Mode chip 仍在 `<ChatInput>` hint row（`<SessionModeChip>`，唯一 mode 入口）。
- **Topbar grammar（V3）**：`Header` = scope-bar(company switcher + compact project selector，左) + 居中 4 peer nav pills(office/sops/market/personnel，active=`accent-surface` + inset `accent-ring`) + 右 iconbar(Activity + Settings icon-button，office 加 1px divider + Studio)。`OfficeToolId` 只剩 `'studio'`（dashboard 已删）。**无铃铛**。
- **运行轴 Board + Live（stage 上方浮层 `StageRunAxisFloats`）**：Board entry toggle 现有 `KanbanTray`（数据/CAS/`officeState.kanbanOpen` 不变，`/kanban`·`/board`·`/k`·⌘J 共用）；Live entry `live-idle`/`live-active`(+pulse)，active 开 `LiveRunOverlay`（Plan + Activity，latency 在 header）。run 结束沉淀进 thread run-record 的**数据契约归 Phase 1**，本壳只对接 Live entry。Kanban 是 `taskTray` 常驻入口（空板也渲染，展开覆盖中心场景上半部五列执行板）。
- **左栏 296px Files/SOPs/Git（`OfficeLeftRail`）**：`.ws-tabs` Files(file tree=`ProjectWorkspaceFiles`) / SOPs(模板列表，点击 → SOPs peer workspace，badge=`sops.length`) / Git(`GitWorkbench` 从右栏 gitSlot 搬来，适配 296px，branch 折进 head)。右栏不再有 Git tab。Git 仅真实本地 repo 状态、diff、local commit、PR-ready compare/unavailable，不做 push/force/假 checks/全局 `Auto-sync`。
- **员工 roster 落 stage（`StageTeamDock`）**：左栏被 Files/SOPs/Git 占用，`AgentPanel` 不再挂左栏；员工 = 场景内 avatar + stage 下方横向 Team dock 条（avatar + name + `STATUS_DOTS[state]` dot + 末尾 Add slot 开员工创建）；选中/inspector 继续走 Personnel routing。
- **Boss Dashboard 入口已删**：`workspace-navigation` 去 `'dashboard'` OfficeToolId、`useOfficeStateBindings` 删 `handleToggleDashboard`/`dashboardOpen`/kanban 互斥、`useAppKeyboardShortcuts` 删 ⌘D（⌘J 保留）、types/url-routing/`useWorkspaceSessionState`/`AppOverlayHost` 删 `dashboardOpen`。`DashboardOverlay` 组件保留无入口（v2 跨项目账本）。single-run cost diegetic，跨项目账本 v1-deferred。**遗留**：chat slash `/dashboard`（`chat-commands.ts` + `ChatPanel.onToggleDashboard` 可选 prop）属 Phase 1 chat-rail，现 shell 不再 wire `onToggleDashboard`，slash 成 no-op，留 Phase 1 清理。
- **Install singularity**：`<InstallDialog>` 位于 desktop renderer 的 `components/app-shell/AppGlobalDialogs.tsx`。所有入口（Market detail CTA / `useDeepLinkInstall` / 文件拖拽 sideload）都过 `useInstallFlow().startRegistryInstall|startFileImport`。新增 install 入口必须复用此 hook。
...
- **Chat attachments end-to-end**：composer 三入口（paperclip / paste / drag-drop）汇 `useChatAttachmentStaging`（sha256 dedupe / 8 MB 单 / 32 MB 总 / 后台 `parseAttachment`；send 复用 staged bytes，不重读 File）；`<ChatInput onSend>` 第二参带 `attachments?: ChatInputAttachmentPayload`，`ChatPanel.handleSend` 调 `persistStagedAttachments` 落盘 → `ChatMessage.attachments` + `RunScope.pendingAttachments`，partial 失败 `rollbackPersistedAttachments` 不留 orphan。桌面端 `TauriAttachmentStore`（5 binary IPC command：`attachment_write/read/list/list_all/delete`，无 base64 inflation）是唯一产品路径。`vaultRef = attachment://<companyId>/<threadId>/<UUIDv4>`。`read_attachment` 是 builtin tool，仅 gateway lane 注入，并且执行时必须匹配当前 `companyId + RunScope.threadId`；跨 company/thread 或缺 run scope 返回 `attachment-forbidden`，不能读 store。Boot 时 `attachmentGcSweeper.run()` 走 `listAll()` 扫 orphan + emit `chat.attachment.gc.swept`；thread/project/company 硬删由 runtime 安装的 attachment cascade 先删 blob 再删 repo row，soft archive 不删。文档解析器 `@offisim/doc-engine/src/import/`（PDF/DOCX/XLSX/PPTX/text/image），`pdf.worker.min.mjs` 由 desktop renderer `prebuild` 拷到 `apps/desktop/renderer/public/`。
- **narrow tier (`390x844`) 口径已被 Tauri-only 决策覆盖**：narrow 只保留为 renderer/component 级响应式不变量；不能再用 standalone web browser surface 作为产品验收目标。Tauri 桌面壳保持 desktop product floor (`minWidth ≥ 1024`)；最终验收只认 release `.app`。

## Key Files

| Area | Entry point | Purpose |
|------|-------------|---------|
| Desktop renderer | `apps/desktop/renderer/src/App.tsx` | Root, workspace routing, runtime init |
| View types | `apps/desktop/renderer/src/lib/app-view-layout.ts` | OverlayKey, OfficeViewMode 类型 |
| Workspace types | `apps/desktop/renderer/src/components/workspaces/types.ts` | WorkspaceKey, OfficeSessionState (`selectedEmployeeId` + `selectedThreadId` 等), layout tier |
| Thread bootstrap | `apps/desktop/renderer/src/hooks/useThreadBootstrap.ts` | 在 active project 上 `ensureProjectHasAtLeastOneThread` + 越界自动重选 |
...
| Git workbench | `apps/desktop/renderer/src/components/git/GitWorkbench.tsx` | Left-rail Git tab (`OfficeLeftRail`); branch/diff/local commit/PR-ready over project workspace_root via `git_exec` |
...
| Personnel routing | `apps/desktop/renderer/src/lib/personnel-routing.ts` | `routeToPersonnel(id, tab)` — single entry for cross-surface employee edit |
| URL routing | `apps/desktop/renderer/src/lib/url-routing/` | parser + serializer + fallback + `useUrlSync`; deep links / Back / Forward 的 canonical path |
...
| Folder picker bridge | `packages/ui-office/src/lib/folder-picker.ts` | `pickWorkspaceFolder` / `revealWorkspaceFolder` SSOT — Tauri dialog + opener plugins; non-Tauri renderer throws `FolderPickerUnavailableError` |
...
- desktop renderer Vite alias 必须与 `ui-office/package.json` exports 同步。新增 subpath export 时必须同步 renderer alias，否则 dev mode 动态 import 会 404。
- **Tauri renderer / non-Tauri debug 双模导入已分叉**: Tauri dev/build 允许真实加载 `@tauri-apps/api/*` / `plugin-fs` / `plugin-sql`；非 Tauri debug 只能走精确 stub。新增 Tauri 依赖时必须同步 stub 或 alias，否则迁移期 renderer debug 会直接炸。
- Dev 端口以 Tauri desktop renderer 5176 与 platform 4100 为准；不要恢复 launcher 端口。
- 修改 `shared-types` 后必须先 `pnpm --filter @offisim/shared-types build`
- **验证命令按依赖顺序串行跑**: `shared-types -> ui-core -> core -> ui-office -> desktop renderer -> desktop`。desktop renderer 读取 `core/dist`，不要和 `core` build 并行跑，否则容易拿到旧产物产生假失败/假通过。
...
- Linux/CI 如需跳过 Tauri，只过滤 `!@offisim/desktop`；launcher 已删除，不再保留 launcher 过滤。
- Three.js 非真实运行时对象可能不完整, 代码里要做 defensive cast / null guard, 不要假设测试环境会替你兜底
- **`pnpm clean` 不清 `tsbuildinfo`**: 各包 `tsconfig.tsbuildinfo` 在包根目录，`pnpm clean` 只清 turbo 缓存 + node_modules。删了 `dist/` 但留下 buildinfo，tsc 当 "no changes" 跳过 emit；turbo 又把残缺 dist 当成功结果缓存（`asset-schema` 自定义 build 脚本是 `generate → tsc → copy`，不走 `tsc-build-if-dist-missing.mjs` helper，最容易撞）。完整 clean rebuild 三件套：`find packages apps -name "*.tsbuildinfo" -delete` + `find . -type d -name ".turbo" -not -path "*/node_modules/*" -exec rm -rf {} +` + `pnpm exec turbo run build --force`。
...
- **desktop 是 Tauri v2 产品壳**: `frontendDist` 必须指向 desktop-owned renderer dist：`../renderer/dist`。
...
- **Desktop builtin tool sandbox**: 实现在 `apps/desktop/src-tauri/src/builtin_tools.rs`（commit `3f618ce9`），暴露给 employee / YOLO tool pool 的 `read_file` / `write_file` / `bash` 都受 workspace_root 约束 + 硬上限：read 8 MB / write 8 MB / bash 默认输出 1 MB / preview 64 KB。这些 builtin 只注入已验证的 Offisim harness/gateway tool path；未验证的 model transport 不能拿到本机工具权限。
...
Open source (MIT), BYO-key. Tauri desktop 是产品表面；provider transport 由本地用户配置驱动，默认不走托管代理。

### Desktop Provider Defaults

- `apps/desktop/renderer/vite.config.ts` dev 模式会从 **repo root `.env.local`** 读取 `MINIMAX_*`，并注入成 `VITE_MINIMAX_*`。
- `packages/ui-office/src/lib/provider-config.ts` 在 **没有本地已保存 ProviderConfig** 时，会自动用 env 起一个 `MiniMax Global` 默认配置。
- 若 UI 没显示 key，不要先假设 env 没读到：优先看当前 provider label / model / live request 是否真走 MiniMax；桌面侧 secure key 可能被 secret store 掩码。

### Live Product Findings

- 历史非桌面 live 记录只作能力背景，不再作为产品验收目标。
- **Model transport / SDK runtime 边界已重新校准（2026-05-09）**：默认 `offisim-core` harness 可以直接通过自己的 model transport / provider adapter 调模型；调用模型不等于进入“SDK lane”，产品上不存在普通 SDK lane。历史代码里仍可能出现 `gateway` / `claude-agent-sdk` / `codex-agent-sdk` / `openai-agents-sdk` 等 execution binding 名称，阅读时按 transport/adapter 字段理解。SDK 只有两种合法身份：一是 Offisim harness 内部 model transport 实现细节；二是显式配置并通过 release `.app` 证据的 SDK-native employee runtime / driver / replacement profile。文件、shell、memory、todo、skill、MCP、builtin tool 不能靠未验证的 model transport 暴露或验收；需要工具证据时走默认 Offisim harness/gateway 工具路径，或已验证的 tool-capable employee profile。SDK-native full-power 路线必须保留原生 tools、MCP、sessions、hooks/guardrails、handoffs/subagents、streaming、cancel、budget、sandbox、telemetry、checkpoint/rollback 证据，不能被压成 one-shot text。
- **MCP truth source（2026-05-17）**：当前口径是 `client ✓; resources/prompts surfacing + Streamable HTTP + OAuth pending`。`McpToolExecutor` 已拥有 stateful tools/resources/prompts/capabilities、abort-aware calls、list-changed refresh 和 shutdown；MCP tool call 仍必须走 `AuditingToolExecutor` + `ToolPermissionEngine`。不要把 resources/prompts UI、Streamable HTTP、OAuth 宣称为已交付。
...
This project is indexed by GitNexus as **Offisim** (31789 symbols, 46503 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
