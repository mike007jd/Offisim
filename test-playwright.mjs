...
try {
  ...
  const drawers = await page.locator('[role="dialog"], [role="complementary"], .drawer, aside').all();
  console.log(`找到 ${drawers.length} 个抽屉/侧边栏`);
  
  const menuButtons = await page.locator('button:has-text("Menu"), button:has-text("菜单"), button:has-text("导航"), [aria-label*="menu" i]').all();
