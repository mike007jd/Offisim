...
> = (
  ...
  (async () => {
    ...
    try {
      ...
      type IndexedTextContent = TextContent & { contentIndex: number };
      type IndexedThinkingContent = ThinkingContent & { contentIndex: number };
      type StreamingBlock = IndexedTextContent | IndexedThinkingContent | StreamingToolCallBlock;
      type StreamingToolCallDelta = NonNullable<
        ChatCompletionChunk.Choice.Delta['tool_calls']
      >[number];

      let textBlock: IndexedTextContent | null = null;
      let thinkingBlock: IndexedThinkingContent | null = null;
      ...
      const finishBlock = (block: StreamingBlock) => {
        ...
        if (block.type === 'text') {
          const finalBlock: TextContent = { type: 'text', text: block.text };
      ...
      const ensureTextBlock = () => {
        if (!textBlock) {
          textBlock = { type: 'text', text: '', contentIndex: blocks.length };
          blocks.push(textBlock);
          stream.push({
            type: 'text_start',
            contentIndex: textBlock.contentIndex,
            partial: output,
          });
        }
        return textBlock;
      ...
      const rawMetadata = (error as { error?: { metadata?: { raw?: unknown } } })?.error?.metadata
        ?.raw;
