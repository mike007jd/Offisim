...
type CacheableContentBlock = ContentBlockParam & {
...
export const streamAnthropic: StreamFunction<'anthropic-messages', AnthropicOptions> = (
  ...
  (async () => {
    ...
    try {
      ...
      const contentIndexByEventIndex = new Map<number, number>();
      const appendBlock = (block: Block) => {
        const contentIndex = output.content.length;
        output.content.push(block);
        contentIndexByEventIndex.set(block.index, contentIndex);
        return contentIndex;
      };
      const blockForEventIndex = (eventIndex: number) => {
        const contentIndex = contentIndexByEventIndex.get(eventIndex);
        if (contentIndex === undefined) return null;
        const block = blocks[contentIndex];
        return block ? { block, contentIndex } : null;
      };
      ...
      for await (const event of iterateAnthropicEvents(response, options?.signal)) {
        if (event.type === 'message_start') {
          ...
          if (event.content_block.type === 'text') {
            ...
            stream.push({ type: 'text_start', contentIndex: appendBlock(block), partial: output });
            ...
            stream.push({
              type: 'thinking_start',
              contentIndex: appendBlock(block),
              partial: output,
            });
            ...
            const block: Block = {
              ...
              arguments: (event.content_block.input as ToolCall['arguments'] | undefined) ?? {},
              partialJson: '',
              index: event.index,
            };
            stream.push({
          ...
          if (event.delta.type === 'text_delta') {
            const entry = blockForEventIndex(event.index);
          ...
          if (entry) {
            const { block, contentIndex } = entry;
            contentIndexByEventIndex.delete(event.index);
            if (block.type === 'text') {
              const finalBlock: TextContent = {
                type: 'text',
                text: block.text,
              ...
              const finalBlock: ThinkingContent = {
                type: 'thinking',
              ...
              const finalBlock: ToolCall = {
                ...
                id: block.id,
                name: block.name,
                arguments: parseStreamingJson(block.partialJson),
