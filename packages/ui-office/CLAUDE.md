...
- `App.tsx` 通过 `useWorkspaceSessionState()` 持有 `activeWorkspace` + per-workspace session state；workspace URL 由 `apps/desktop/renderer/src/lib/url-routing/` parser/serializer + `useUrlSync()` 统一维护。不要恢复旧的 `view` 双状态或内部 workspace history stack。
...
- **`<StatusBar>` 三槽接口（`dashboardSlot` / `notificationSlot` / `gitBranchSlot`）**：apps/desktop/renderer 通过 `<AppMainShell>` 喂 Dashboard 切换 chip + `<NotificationCenter>` + `useGitBranch` 真实分支。**禁止占位假数据**（commit `279760cb` 之前 chip 显示 `main` 不论真值）。git branch desktop renderer 端必须 fallback 成 `null` → 槽返回 `null` → `StatusBar` 不渲染。
...
- `CeremonyHost` (`apps/desktop/renderer/src/components/office-shell/OfficeSceneSurface.tsx`) 隔离 ceremony state, 不要把 `useSceneOrchestrator` 上提到 App
- `SceneCanvas` 的 force-2D fallback 由单一 `useReducer` 管理 (`reportCrash` / `fpsTierOff` / `requestRetry` / `viewModeBumped` 四个 action)。`reportCrash` bumps crashCount，`fpsTierOff` 是 perf 信号不 bump。3D crash / perf fallback 必须通知父层同步 `viewMode='2D'`，禁止出现 header 仍选 3D 但实际渲染 2D 的 ghost-state。显式切回 3D 必须 reset：父层 `viewModeNonce: number` 每次 toggle 点击 +1（含 same-value clicks），SceneCanvas 同时监听 `viewMode` 从 2D→3D 触发 `viewModeBumped`，覆盖快捷键/URL 等非 header click 入口。`<SceneFallbackBadge>` 只作为瞬态兜底，不得替代父层 viewMode 同步，颜色走 warning tokens。
...
- SOP: 选中 SOP 后是四区 builder shell：左 `SopSidebar`、中 `SopDagCanvas`、右 `SopInspectorPanel`、底部 `SopNlCommandBar` 同屏；inspector 只读，编辑仍走节点双击 / 右键菜单。DAG ports 必须每次渲染，非 edit mode 用低透明 + 不抢 pointer，edit mode 才激活 drag-to-connect。Tauri release `.app` 下，`SopDagCanvas` 不得把 node face 放回 SVG `foreignObject`；卡片用 HTML overlay，ports / node drag rect / edge disconnect hit area 用 SVG interaction layer，并共享同一 graph translate/scale。`handleRun` 执行前校验 role_slug 存在性, 缺失角色时 warning toast
...
- Chat attachment store：`AttachmentStore.storageAvailable` 是当前可用性字段，`idbAvailable` 仅兼容旧调用。Desktop `TauriAttachmentStore` 持久化走 Rust IPC；`listAll()` 必须调用 Rust `attachment_list_all`，不能把 Blob 拉进 JS heap。`StagedAttachment.bytes` 是 send-time 写入来源，不要再二次 `file.arrayBuffer()`。
...
- Skill install outcome 双面 SSOT：copy 由 `@offisim/shared-types.skillInstallOutcomeLabel(outcome)` 唯一产出。chat assistant message (`apps/desktop/renderer/src/runtime/interaction-follow-up.ts`) 和 activity rail (`subscribeInteractionMappers` 订阅 `SKILL_INSTALL_OUTCOME`) 都消费同一函数；slug 由 `SkillInstallCommitter` 从 `row.slug` 填进 outcome，render 时不查 repo。
...
- **Web vite stub**：`apps/desktop/renderer/src/polyfills/tauri-plugin-{dialog,opener}.ts` 是 noop 函数，vite alias + `optimizeDeps.exclude` 都已加；新增其他 Tauri plugin 时按同款补 stub，否则 web dev 动态 import 会 404。
