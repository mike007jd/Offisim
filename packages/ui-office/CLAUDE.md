...
- **`ProjectWorkspaceFiles.tsx`** 是 workspace_root 文件树 UI，挂在 project picker selected summary 内。桌面通过 `project_list_dir` 列目录、`project_read_file` 读文本预览；两者都受 Rust builtin_tools workspace_root sandbox 约束。浏览器只显示 `Desktop files only`，不调用 Tauri API。
