...
- Chat attachment store：`AttachmentStore.storageAvailable` 是当前可用性字段，`idbAvailable` 仅兼容旧调用。Web `WebAttachmentStore` 使用 IDB v2（`blobs` + `metas`），`listAll()` 只能走 meta-only store，不能用 `getAll()` 把 Blob 拉进 heap；Tauri `listAll()` 必须调用 Rust `attachment_list_all`。`StagedAttachment.bytes` 是 send-time 写入来源，不要再二次 `file.arrayBuffer()`。
