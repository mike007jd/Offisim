...
import { ARCHETYPE_ICONS, LOCK_ICON_PATH, getFloorPatternId } from './editor/archetype-visuals.js';
import type { EditorZone, PlacedItem } from './editor/types.js';
import { SCALE, SVG_H, SVG_W, editorZoneRect, prefabColor, toSVG } from './editor/types.js';
    ...
    <div className="flex h-12 shrink-0 items-center justify-between border-b border-white/[0.06] px-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1 mr-2">
          <button
            type="button"
            onClick={ed.handleZoomOut}
            className="rounded p-1 text-zinc-600 hover:bg-white/[0.06] hover:text-zinc-400"
          >
            <ZoomOut className="h-3.5 w-3.5" />
          </button>
          <span className="font-mono text-[9px] text-zinc-500 w-8 text-center">
            {Math.round(ed.zoom * 100)}%
          </span>
          <button
            type="button"
            onClick={ed.handleZoomIn}
            className="rounded p-1 text-zinc-600 hover:bg-white/[0.06] hover:text-zinc-400"
          >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          return (
            <div key={group.archetype}>
              <button
                type="button"
                onClick={() =>
                  ed.setCollapsed((p) => ({ ...p, [group.archetype]: !p[group.archetype] }))
                }
        ...
        <div className="border-t border-white/[0.06] mt-1 pt-1">
          <button
            type="button"
            onClick={() => {
              ed.setShowCustomForm((v) => !v);
              ed.setPlacingPreset(null);
            }}
          ...
          {ed.showCustomForm && (
            <div className="px-3 pb-2 space-y-2">
              <input
                type="text"
                value={ed.customLabel}
                onChange={(e) => ed.setCustomLabel(e.target.value)}
              ...
              />
              <select
                value={ed.customArchetype}
                onChange={(e) => ed.setCustomArchetype(e.target.value as 'workspace')}
                className="w-full rounded border border-white/10 bg-white/[0.04] px-2 py-1 font-mono text-[10px] text-zinc-200 outline-none focus:border-blue-500/50"
              >
      ...
      {ed.placingPreset && (
        <div className="border-t border-white/[0.06] px-3 py-2 bg-blue-500/5">
          <p className="font-mono text-[9px] text-blue-400">
            Placing: <strong>{ed.placingPreset.label}</strong>
          </p>
    ...
    >
      ...
      >
        ...
        {ed.placingPreset && ed.ghostPos && (
          <GhostPreview
            preset={ed.placingPreset}
            ghostPos={ed.ghostPos}
            overlaps={ed.ghostOverlaps}
          />
        )}
      </svg>
    </div>
...
function GhostPreview({
  ...
  ghostPos,
  overlaps,
}: { preset: ZonePreset; ghostPos: { x: number; y: number }; overlaps: string[] }) {
...
function ZoneProperties({ ed }: { ed: Ed }) {
  ...
  return (
    <div
      className={`shrink-0 border-l border-white/[0.06] bg-[#060a14] flex flex-col transition-all duration-200 overflow-hidden ${z ? 'w-64 opacity-100' : 'w-0 opacity-0 border-l-0'}`}
    >
      {z && (
        <>
          <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3">
            ...
            <button
              type="button"
              onClick={() => ed.setSelectedZoneId(null)}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                value={z.label}
                onChange={(e) => ed.handleLabelChange(e.target.value)}
                  ...
                  <div key={axis}>
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      <button
                        type="button"
                        onClick={() =>
                          ed.handleMoveZone(axis === 'X' ? -1 : 0, axis === 'Z' ? -1 : 0)
                        }
            ...
            <div>
              ...
              <p className="font-mono text-xs text-zinc-300">
                {ed.itemsByZone.get(z.id)?.length ?? 0} items
              </p>
              {z.deskSlots > 0 && (
                <p className="font-mono text-[9px] text-zinc-500 mt-0.5">
                  {z.deskSlots} desk slots
                </p>
              )}
            </div>
