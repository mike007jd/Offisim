...
const ROW_BASE =
  'group flex h-auto w-full items-center justify-between gap-2 rounded-md border border-transparent px-2 py-1.5 text-left text-caption transition-colors';
  ...
  return (
    <div className="flex flex-col gap-1 px-2 py-2">
      ...
      <div className="flex flex-col gap-0.5">
        {threads.length === 0 ? (
          <div className="px-2 py-2 text-caption text-text-muted">No threads yet.</div>
        ) : (
          threads.map((t) => {
  ...
  return (
    <div className="flex flex-1 items-center gap-1">
      <Input
        ...
        className="h-8 flex-1 rounded border-border-default bg-surface-default px-2 py-1 text-caption text-text-primary focus:border-border-focus"
      />
      <Button
        type="button"
        ...
        variant="ghost"
        size="icon"
        className="size-7 text-text-secondary hover:bg-surface-hover hover:text-text-primary"
        aria-label="Save"
      >
