...
export function RightSidebar({
  ...
  const workflowLabel = useMemo(() => {
    if (!stage && !isRunning) return 'Idle. Collaboration happens here.';
    if (!stage && isRunning) return 'Starting workflow...';
    const activeStage = stage ?? 'boss';
    return `${STAGE_META[activeStage].chatLabel} Task context stays in this rail.`;
  }, [stage, isRunning]);

  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="border-b border-white/5 px-3 py-3">
        <p className="text-[10px] uppercase tracking-[0.24em] text-slate-500">Collaboration</p>
        <p className="mt-1 text-xs leading-relaxed text-slate-300">{workflowLabel}</p>
      </div>
