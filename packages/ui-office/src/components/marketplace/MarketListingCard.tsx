...
export function MarketListingCard({ listing, onClick }: MarketListingCardProps) {
  ...
  return (
    <button
      ...
      className={`group flex h-[220px] flex-col rounded-2xl border bg-white/[0.04] p-5 text-left shadow-md transition-all hover:bg-white/[0.06] hover:border-opacity-60 ${rarity.border} ${rarity.glow}`}
    >
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-slate-500 truncate ml-2">@{listing.creator.handle}</span>
      </div>

      <h3 className="mt-3 text-base font-bold text-white truncate">{listing.title}</h3>
      <p className="mt-1 flex-1 text-sm leading-relaxed text-slate-400 line-clamp-2">
        {listing.summary}
      </p>
