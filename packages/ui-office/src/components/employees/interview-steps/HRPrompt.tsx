...
export function HRPrompt({ step }: HRPromptProps) {
  return (
    <div className="flex items-start gap-3">
      {/* HR Avatar */}
      <div className="flex-shrink-0 w-10 h-10 rounded-full bg-sea-blue/30 border-2 border-sea-blue flex items-center justify-center">
        <span className="text-xs font-pixel-mono font-bold text-sea-blue">HR</span>
      </div>

      {/* Speech Bubble */}
      <div className="relative bg-ocean-mid border-2 border-ocean-light p-3 flex-1">
        {/* Bubble arrow */}
        <div className="absolute left-[-8px] top-3 w-0 h-0 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent border-r-[8px] border-r-ocean-light" />
        <div className="absolute left-[-5px] top-3 w-0 h-0 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent border-r-[8px] border-r-ocean-mid" />
        <p className="text-sm text-sand leading-relaxed">{HR_QUESTIONS[step]}</p>
      </div>
    </div>
  );
}
