...
/**
 * Per-execution chat run scope. Generated by `ChatPanel.handleSend` and threaded
 * through `sendMessage → orch.execute → graph config.configurable.runScope`.
 *
 * Every chat-session-store write action takes the scope explicitly so that an
 * async resolution from a stale run becomes a no-op rather than a cross-write.
 */
