...
function WorkstationSVG({ x, y }: { x: number; y: number }) {
  return (
    <g transform={`translate(${x}, ${y})`}>
      <rect x="-22" y="-18" width="44" height="36" rx="2" fill="var(--surface-lighter)" stroke="var(--surface-mid)" strokeWidth="1" />
      ...
      {[[-10, -8], [10, -8], [-10, 8], [10, 8]].map(([cx, cy]) => (
        <circle key={`${cx}-${cy}`} cx={cx} cy={cy} r="3" fill="var(--surface-mid)" stroke="var(--surface-lighter)" strokeWidth="0.5" />
      ))}
    </g>
  );
}
    ...
    <g transform={`translate(${x}, ${y})`}>
      ...
      {[0, 1, 2, 3].map((shelf) => (
        <g key={shelf}>
          ...
          {[0, 1, 2, 3, 4, 5, 6].map((b) => (
            <rect key={b} x={-21 + b * 6.5} y={-28 + shelf * 17} width="5" height="14" rx="0.5" fill={colors[(shelf * 7 + b) % colors.length]} />
          ))}
        </g>
      ))}
    </g>
  ...
  return (
    <g transform={`translate(${x}, ${y})`}>
      ...
      {[0, 72, 144, 216, 288].map((angle) => (
        <path key={angle} d="M0,0 C-12,-18 12,-18 0,0" fill="#10b981" transform={`rotate(${angle})`} />
      ))}
    </g>
  );
}
...
function WhiteboardSVG({ x, y }: { x: number; y: number }) {
  return (
    <g transform={`translate(${x}, ${y})`}>
      <rect x="-30" y="-20" width="60" height="40" rx="2" fill="#f1f5f9" stroke="var(--surface-mid)" strokeWidth="1" />
...
function GenericPrefabSVG({ x, y, label, color }: { x: number; y: number; label: string; color: string }) {
  return (
    <g transform={`translate(${x}, ${y})`}>
      <rect x="-15" y="-15" width="30" height="30" rx="3" fill={color} fillOpacity={0.15} stroke={color} strokeWidth="1" strokeOpacity={0.4} />
      <text x="0" y="3" textAnchor="middle" fontSize="7" fill={color} fontWeight="600" fontFamily="system-ui">{label.slice(0, 3)}</text>
    </g>
  );
}
