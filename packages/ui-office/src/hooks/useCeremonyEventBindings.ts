...
import { UNASSIGNED_ZONE_ID, type Zone, resolveZoneForRole } from '@offisim/shared-types';
...
import { getNextSlot, getRestPos, resetSlotCounters } from '../runtime/zone-slot-counter';
import {
  computeMtgPositions,
...
export function useCeremonyEventBindings({
  ...
  const startDismissPhase = useCallback(
    ...
    [ceremonyVersionRef, moveEmployeeToRest, safeTimeout, scheduleCeremonyReset, setCeremony],
  );
  ...
  const gatherAll = useCallback(
    ...
    [clearAssignedSceneState, setCeremony],
  );
  ...
  const dispatchEmployee = useCallback(
    ...
    [ceremonyVersionRef, getSceneObstacleFootprints, safeTimeout, setCeremony],
  );
