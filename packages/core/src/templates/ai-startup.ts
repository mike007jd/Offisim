    ...
    },
    {
      ...
      steps: [
        ...
        },
        {
          ...
          role_slug: 'frontend',
          instruction:
            'Build the AI feature end-to-end following the ux_design and feature_spec. Implementation requirements: (1) API integration — connect to the model inference endpoint with proper authentication, retry logic (exponential backoff), and timeout handling, (2) Streaming UI — implement real-time content display for streaming responses with proper buffering and render optimization, (3) State management — handle all states from the UX design (loading, streaming, complete, error, low-confidence) with smooth transitions, (4) Error boundaries — graceful degradation at every level, never show raw API errors to users, (5) Telemetry — instrument key events (request sent, first token, complete, error, user feedback) for monitoring, (6) Live verification — exercise the full user flow in the real runtime and report the observed behavior for each state. Output the complete implementation with a verification report.',
