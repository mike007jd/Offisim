...
describe.skipIf(!HAS_MINIMAX)('Middleware smoke — recordedLlmCall (MiniMax)', () => {
  it('creates a DB record with real token counts, latency, and no error', async () => {
    ...
    if (!record) throw new Error('Expected recorded LLM call');
...
describe.skipIf(!HAS_MINIMAX)('Middleware smoke — recordedLlmStream (MiniMax)', () => {
  it('creates a DB record + emits events + streams chunks', async () => {
    const gateway = createMiniMaxGateway();
    ...
    if (!finalChunk) throw new Error('Expected final stream chunk');
    ...
    expect(calls).toHaveLength(1);
    const record = calls[0];
    if (!record) throw new Error('Expected recorded stream call');
    ...
    if (!completedEvent) throw new Error('Expected completed event');
    expect(completedEvent.payload.latencyMs).toBeGreaterThan(0);
    expect(completedEvent.payload.inputTokens).toBeGreaterThan(0);
    expect(completedEvent.payload.outputTokens).toBeGreaterThan(0);
...
describe.skipIf(!HAS_MINIMAX)('Middleware smoke — message pruning (MiniMax)', () => {
  it('prunes 60+ messages to <=52 and still gets a valid response', async () => {
    const gateway = createMiniMaxGateway();
    const { runtimeCtx, repos } = makeRealCtx(gateway);
    ...
    expect(calls).toHaveLength(1);
    const record = calls[0];
    if (!record) throw new Error('Expected recorded pruned-message call');
    expect(record.input_tokens).toBeGreaterThan(0);
    expect(record.output_tokens).toBeGreaterThan(0);
    expect(record.error_code).toBeNull();
