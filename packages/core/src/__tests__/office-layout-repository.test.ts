...
describe('MemoryOfficeLayoutRepository', () => {
  ...
  it('findActive returns active layout', async () => {
    ...
    await repos.officeLayouts.create({
      layout_id: 'l-1', company_id: 'c-1', name: 'A', layout_json: '{}', is_active: 0,
    });
    await repos.officeLayouts.create({
      layout_id: 'l-2', company_id: 'c-1', name: 'B', layout_json: '{}', is_active: 1,
    });
  ...
  it('setActive deactivates others', async () => {
    const repos = createMemoryRepositories();
    await repos.officeLayouts.create({
      layout_id: 'l-1', company_id: 'c-1', name: 'A', layout_json: '{}', is_active: 1,
    });
    await repos.officeLayouts.create({
      layout_id: 'l-2', company_id: 'c-1', name: 'B', layout_json: '{}', is_active: 0,
    });
  ...
  it('updates layout name and json', async () => {
    const repos = createMemoryRepositories();
    await repos.officeLayouts.create({
      layout_id: 'l-1', company_id: 'c-1', name: 'Old', layout_json: '{}', is_active: 1,
    });
  ...
  it('findByCompany returns only matching company', async () => {
    ...
    await repos.officeLayouts.create({
      layout_id: 'l-2', company_id: 'c-2', name: 'B', layout_json: '{}', is_active: 1,
    });
