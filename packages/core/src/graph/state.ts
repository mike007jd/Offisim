...
/**
 * Per-execution chat run scope. Generated by `ChatPanel.handleSend` at user-submit
 * time and threaded through `sendMessage → orch.execute → graph config.configurable.runScope`.
 *
 * Lives in `config.configurable.runScope` only — never on long-lived state
 * (would re-introduce singleton cross-pollution at one layer up).
 */
