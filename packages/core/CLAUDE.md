...
- **2026-04-29 runtime remediation**：Tauri `desktop-trusted` + `gateway` lane 现在会给 employee / YOLO tool pool 注入受项目 `workspace_root` 约束的内置 `read_file` / `write_file` / `bash`；web/browser-limited 不注入。`claude-agent-sdk` / `codex-agent-sdk` / `openai-agents-sdk` lane 在 Offisim 1.0 口径下是 text/reasoning-only：runtime 必须设置 `llmToolCallsEnabled=false`，不暴露 file/shell/memory/todo/skill/MCP/builtin tool schema；adapter 收到任何 tool request 必须失败关闭，不能假装已执行。
- **本地工具路由硬规则**：凡是任务文本要求 workspace/file/path/read/write/bash/shell/command/pnpm/npm/cargo/timeout/越界拒绝，Boss / Manager / PM preflight / direct setup 都必须只选 enabled internal employee。external A2A 员工没有本机工作区能力，不能作为这类任务的证据来源；直聊 external A2A 做本地工具任务必须 fail fast。
