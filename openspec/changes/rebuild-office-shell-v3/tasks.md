...
- [x] 6.3 release `.app` live（用户/Codex）：无底部状态栏 / cost readout diegetic + live 高亮 / 通知 `.nb-dot` 角标无铃铛 / stage 无 Board/Live 浮动 tab / `.stage-pipe` run-state + Stop / pending-interaction 仍可见 / 左栏 Files-SOPs-Git / 员工 Team dock + 场景 avatar + Add slot / topbar 无铃铛 / Dashboard 入口消失
  - **Earlier attempt 2026-05-24**：release `.app` 已用当前 worktree 精确路径构建并启动，但本机处于 macOS 锁屏界面；CGWindow 可见 Offisim 窗口存在，Computer Use 附着返回 `cgWindowNotFound`，解锁后改用同一 `.app` 路径补跑，未用 dev server 或浏览器替代。
  - **First live sweep 2026-05-24**：当前 release `.app` 精确路径已重新构建并用 Computer Use 附着成功。已确认：界面内假 macOS 红绿灯不存在（只保留系统窗口按钮）、无底部全局 StatusBar、stage 顶部 Board/Live floating tab 不再出现、Team dock 铺满底部且含 Add slot、左栏 Files/SOPs/Git 可见、topbar 走 scope/nav/iconbar 且无 header bell、Dashboard 入口不可见、PerformanceHUD/FPS 调试框不再污染 release。running `.stage-pipe` Stop/Resume、pending-interaction 可见性、live 高亮随后在 PASS 记录补齐。
  - **PASS 2026-05-24**：当前 release `.app` 重新构建于 `/Users/haoshengli/Seafile/WebWorkSpace/Offisim/apps/desktop/src-tauri/target/release/bundle/macos/Offisim.app`，Computer Use 附着 pid `95160`。真实 HIL 任务触发 `.stage-pipe` running pill（`ANALYZING` + `STOP`），Stop 后进入 `Stopped at ROUTING/PLANNING` + `DISCARD`，右栏 assistant-ui timeline 显示 interrupted run record。随后同一 release bundle 触发 plan-review pending：stage pill 显示 `Review plan before execution` + `NEEDS INPUT`，右栏显示 `Review plan before execution` prompt + `Start execution / Revise plan / Cancel`，scene 内状态为 `Waiting for plan review...`。这次修正了 `StagePipe` 在 `isRunning=false && pendingInteraction` 时直接消失的问题，pending cue 不再依赖 graph running 标志。既有 shell 项（无底部 StatusBar、无 Board/Live 浮动 tab、无 header bell、Dashboard 入口消失、左栏 Files/SOPs/Git、Team dock + Add、diegetic cost/notification）保持通过。
- [x] 6.4 archive gate 三查 + 协议台账（Tauri git_exec 路径无变化则不动台账）
  - **PASS 2026-05-24**：spec consistency：`openspec validate rebuild-office-shell-v3 --strict` 通过；tasks consistency：6.3 live gap 已用 release `.app` + Computer Use 补齐并从 unchecked 改为 checked；docs/comments consistency：root `CLAUDE.md` / `packages/ui-office/CLAUDE.md` 已锁定无 StatusBar、无旧 Board/Live scene float、right rail assistant-ui 单轴、pending 进入 StagePipe + right rail。协议台账不动：本 change 未碰 Tauri/A2A/MCP/git_exec 路径。
