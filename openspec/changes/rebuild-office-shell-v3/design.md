...
V3 prototype `:289` `grid-template-columns: 296px minmax(620px,1fr) 448px`；`.scene-cost`/`.sc-notif`(:124-133)；`.stage-runaxis`/`.stage-entry`(:662-684)；`.gw-*` git widget(:540-597)；左栏 `.ws-tabs` File/Git/SOP。

## Goals / Non-Goals

**Goals:** shell = V3（删状态栏/铃铛count/Dashboard 入口；加 diegetic cost readout + 通知 dot + 运行轴 Board+Live；左栏 File/Git/SOP；topbar grammar；296/448）。
...
移除 `StatusBar.tsx` + `AppMainShell` 渲染块 + AppLayout statusBar slot。新增 `SceneCostReadout`（stage 内 absolute right/bottom，消费 `useDashboardMetrics`，`.live` 时 accent-ring + pulse dot）。run-state（stage/pending interaction）并入 cost readout 或 run-axis Live entry。
...
### D3 — 运行轴 Board+Live 上舞台
新增 `StageRunAxisFloats`（stage 上方居中）：Board entry（toggle 现有 KanbanTray，persistent）、Live entry（idle/active，pulse dot；运行中广播，run 结束 → 沉淀进 Phase 1 的 thread run-record）。KanbanTray 数据/CAS 不变，仅入口从 taskTray chip 收进运行轴。
**Open**：Live 广播数据源与 run-record 的边界（与 Phase 1 对齐：Live=运行中瞬态，run-record=沉淀态）。
...
single-run cost 进 diegetic `.scene-cost` readout + Live overlay 单 run 成本明细；跨项目账本 v1-deferred（无替代、不给假家）。`DashboardOverlay` 组件保留无入口（v2 跨项目账本）还是随删——apply 时定，倾向保留组件、删入口。
...
- Office rail seam（vs Phase 1 `rebuild-chat-rail`）：**Phase 2 OWNS** rail 宽度/位置（296/448）+ 左栏 Files/SOPs/Git + StatusBar 删除 + stage Live **entry shell** + active-state + `.stage-pipe` run-state/Stop。**Phase 1 OWNS** 右栏内列（thread / composer / run-record + composer model-chip）。「run 结束沉淀进 thread run-record」的 **DATA CONTRACT 归 Phase 1**——Phase 2 只 spec Live entry shell + active state，并显式 DEFER sediment 数据契约给 Phase 1，不在此 change 断言/实现无法在此完成的 sediment 行为。

## Risks / Trade-offs

- **删 StatusBar 波及 slot 提供方 + hooks**（useDashboardMetrics/useGitBranch/RunState/Work/Resources segments）→ 逐个迁移：EnergyMeter(token+cost) → `.scene-cost` readout；run-state headline + Stop → stage `.stage-pipe`；latency → Live overlay header；model name → composer model-chip(Phase 1)；pending-interaction label → Live 结构化 entry + 右栏 interaction bubble / HIL modal(Phase 1/lifecycle)。run-state headline + Stop + pending-interaction 都不能丢。
- **Live 广播 vs run-record 边界**（跨 Phase 1）→ Phase 2 只做 Board + Live entry 壳 + active state；sediment 数据契约归 Phase 1，本 change 不断言 sediment 行为。
...
4. StageRunAxisFloats（Board toggle + Live 壳）。
...
- Live overlay 单 run cost / activity 数据源 ↔ Phase 1 run-record sediment 数据契约的对接细节（Phase 1 owns 契约；Phase 2 只对接 Live entry 壳）。
