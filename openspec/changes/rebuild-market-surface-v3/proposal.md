...
V3 设计稿把 Market 从「Steam-store hero 卡」改成「密集 inventory chip」：卡片走 `h-market-listing-card`、cover 是 **88px band** 渲染 **per-kind cover 可视化**（kv-employee/kv-skill/kv-sop/kv-template/kv-layout/kv-prefab/kv-bundle，按 `listing.kind` 选），不渲 SaaS 16:9 hero 图；无 kind viz 命中时（legacy/spec-thumbnail）fallback icon-only tile（48×48 居中）；grid `repeat(auto-fill,minmax(252px,1fr))` gap14 pad sp-7、featured 卡 `grid-column: span 2` + rarity radial cover；filter bar 48px 用 `.nav` chip-grammar 的 segmented（无原生 select 箭头）；detail 从右侧 side-panel 440px 滑入（替换当前两栏 fr split）；rarity 用 CSS var 别名（employee→accent/skill→violet/sop·prefab→warn/template→violet/layout→danger/bundle→ink-3，含 `--r-<kind>-s` surface 别名）；stats row stars(warn)·installs(mono)·creator。
...
- **listing card → 密集 chip**：`--r-md` 边、`--line-soft` border、`--elev-1`；cover 是 **88px band 渲 per-kind cover 可视化**（按 `listing.kind` 选 `kv-<kind>`），band 由 rarity surface `--rcs` 着色，featured 用 rarity radial-gradient cover；**不渲 16:9 image hero**；**无 kind viz 命中时 fallback icon-only tile（48×48 kind icon 居中）**；curated/featured 才可附加 ≤96×64 小 thumb（非全幅）。保留 kind chip overlay + `Installed` badge（仍按 `listing_id` 或 `package_id::version` 匹配）+ creator handle + verification dot + stats row（stars=warn、installs=mono、creator）+ 顶部 rarity stripe（`var(--rc)`）。
...
- **rarity → CSS var**：`market-rarity.ts` 从 Tailwind class 改 CSS var 别名映射（`--r-<kind>` + 配对 surface `--r-<kind>-s`），卡片顶 3px stripe + badge 用 `var(--rc)`、cover band tint 用 `var(--rcs)`。
...
- 验收 gate：typecheck + 串行 build；release `.app` live 验：卡片密集 / 88px band 按 kind 渲 per-kind cover viz（非 icon-only）/ grid 密度 minmax252 + featured span2 / filter segmented 无原生箭头（mode/manageTab 仍 EntityDropdown）/ detail 右侧 440 滑入 / rarity 颜色 + cover tint / install 按钮仍只 employee+skill / 已装 badge（含 detail 打开时 re-seed 后仍在）/ deep-link install 正常。
