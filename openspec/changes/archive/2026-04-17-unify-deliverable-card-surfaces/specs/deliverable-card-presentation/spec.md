...
- `formatTimeAgo(ts: number): string` — maps an epoch-ms timestamp to short relative labels (`just now` / `Nm ago` / `Nh ago` / `Nd ago`).
...
The `Deliverable` interface returned by `useDeliverables()` SHALL carry a `contentSize: number` field, populated from `artifact.content.length`. Consumers SHALL NOT re-derive byte-size at render time.

#### Scenario: Hook row carries contentSize
- **WHEN** `useDeliverables()` mounts and returns rows
- **THEN** each row has a `contentSize` number equal to its `artifact.content.length`
...
The three sub-tabs SHALL render as a pill-style tab bar consistent with the top-level Chat/Tasks tab styling (border-radius-full, cyan active state). The Deliverables section block (`RightSidebar.tsx:138-145` in the pre-change code) SHALL be removed; its content moves entirely into the Outputs sub-tab.
...
`packages/ui-office/src/components/kanban/KanbanBoard.tsx` SHALL NOT render a "Deliverables" column. The pre-change column at lines 201-234 (and the `useDeliverables` import + derived `deliverables` variable) SHALL be removed. KanbanBoard's role is step-by-step plan-progress visualization only; deliverable listing is delegated to the Outputs sub-tab.
