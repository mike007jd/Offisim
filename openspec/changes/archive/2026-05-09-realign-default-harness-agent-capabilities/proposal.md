...
- State that the default harness can call models directly. SDK use inside that path is only a model transport/provider-adapter implementation detail, not a product lane.
- Split model transport from employee agent runtimes: a configured employee agent profile may be a complete agent runtime when it has evidence; ordinary SDK lane is not a product concept.
...
- Clean stale wording in docs, specs, code comments, and user-facing copy that says or implies "calling a model means selecting an SDK lane" or "all tool work must forever be gateway-only."
...
- `harness-model-transport-boundary`: Defines direct model calling under `offisim-core` and separates model transport from product runtime routes.
...
- `agent-sdk-provider-lanes`: Recasts legacy lane wording as model transport binding only, and removes "ordinary SDK lane" as a product architecture concept.
...
- `provider-lane-matrix`: Separates model transport evidence from employee runtime/control-plane evidence.
