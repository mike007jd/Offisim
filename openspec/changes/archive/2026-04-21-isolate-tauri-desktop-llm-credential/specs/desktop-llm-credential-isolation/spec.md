...
On Tauri desktop, the provider credential is stored in a **Rust-only plaintext file** at `<app_local_data_dir>/runtime_secret.txt` (Unix mode `0600`, atomic tmp-file + rename on write). Rationale: the earlier Keychain-backed plan required either Developer ID signing (production) or repeated ACL re-authorization on every unsigned rebuild (dev), and both failure modes would prompt the user; Claude Code's own `secureStorage` also falls back to plaintext for the same reason (`src/utils/secureStorage/{plainTextStorage.ts,fallbackStorage.ts}`). The threat model here is webview prompt-injection, not local-disk exfiltration — process-level file isolation is sufficient.

The secret SHALL NOT be readable from the webview. The desktop secret storage command surface SHALL expose only `runtime_secret_status` / `runtime_secret_set` / `runtime_secret_clear`. **No `_get` / `_read` / `_peek` variant may be added.** The only Rust code that dereferences the file contents SHALL be the transport layer (`apps/desktop/src-tauri/src/llm_transport.rs`) immediately before injecting it into an outbound `reqwest` request — after injection the credential SHALL be dropped (no storage on per-connection state, no logging).
...
### Requirement: `llm_fetch` is the sole LLM HTTP transport on Tauri

All Tauri-mode outbound LLM HTTP traffic — any request made by `AnthropicAdapter` / `OpenAiAdapter` (both the `openai` and `openai-compat` branches) — SHALL route through the Rust `llm_fetch` Tauri command via a custom `fetch` injected into SDK client options at `createTauriRuntime` time. Direct `globalThis.fetch(<provider-endpoint>)` / `new XMLHttpRequest()` / any other transport from webview to `api.anthropic.com` / `api.openai.com` / `api.minimax.io` / compat endpoints SHALL NOT occur in the Tauri code path.

`SubscriptionAdapter` (ACP) is exempt — it spawns `claude` via `node:child_process`, carries no HTTP credential, and is not in transport scope.

#### Scenario: Single transport hook in Tauri factory
- **WHEN** auditing `apps/web/src/lib/tauri-runtime.ts`'s `createGateway({...})` call
- **THEN** `fetch: createTauriLlmFetch(...)` is passed in every non-`subscription` branch
...
- **AND** Rust `llm_fetch` sets `Authorization: Bearer <keychain-secret>` header before dispatching
...
- **THEN** Rust sets `x-api-key: <keychain-secret>` header
...
- **WHEN** Settings saves a new provider (user `setRuntimeSecret` overwrites the secret file) and runtime reinits
...
### Requirement: AbortSignal propagates to Rust-side reqwest cancellation

When the SDK's request is aborted (user cancels / orchestrator tears down / timeout fires), the `AbortSignal` attached to the TS-side custom fetch response SHALL trigger an `invoke('llm_fetch_abort', { requestId })` call. The Rust side SHALL cancel the in-flight `reqwest` via a per-request `tokio_util::sync::CancellationToken`, closing the Channel with no further emissions. Abort SHALL be idempotent — aborting a completed / not-yet-started / already-aborted request SHALL return Ok without error.
...
- **AND** the TS-side ReadableStream's reader receives `controller.error(AbortError)` (or equivalent Web-Fetch-API-shaped abort error)

#### Scenario: Abort after completion is no-op
- **WHEN** a request completes (Done event fired) and the caller then aborts the already-resolved signal
