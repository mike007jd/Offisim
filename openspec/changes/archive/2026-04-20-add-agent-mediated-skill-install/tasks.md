...
- [x] 9.1 Web MiniMax + 内置 employee → 用户说 "装一下 github.com/anthropics/skills 里的 do-research" → 工具调用 → preview bubble 出 → 看到 SKILL.md 正文 + allowedTools + 源 URL → confirm → skills 表出 company-scope 行、vault 出 `companies/{id}/skills/do-research/SKILL.md` — **verified 2026-04-20**（Codex headed Chrome + Playwright；`skills/frontend-design` 代 `do-research`，repo 实路径；confirm bubble / Source git+subpath / Scope `Company (all employees)` / `Skill installed.` 全 PASS）
- [x] 9.2 Web 对同一员工说 "装到 Alice 那儿" → scope 推成 employee + targetEmployeeId 解析 → preview 显示 `Scope: Employee: Alice` → confirm → skills 表出 employee-scope 行 — **verified 2026-04-20**（commit `8fccdf82` 补 A `employee-prompt-assembly` 注入 coworker 名册 + B `skill-install-tools` 接受 id 或 name fuzzy match 后；Codex 用 Maya Lin 重验主路径 PASS：`Scope: Employee: Maya Lin` + `Skill installed.`。B ambiguous 分支无 live 证据——A 生效时 LLM 直接从名册拿 id 走 findById，不进 name fallback；留给 prompt 截断 / 历史 message name 字符串 / agent 间消息兜底场景）
...
- [ ] 9.4 Desktop 用户上传一个 zip（含 SKILL.md + scripts/）→ preview 显示 assets 列表 → confirm → vault 出对应 scripts 文件 — **observation 2026-04-20**：chat panel 无 attach-file affordance，只有 DevTools 程序化路径；web 不暴露 uploadResolver 到 `__OFFISIM_DEBUG__`，desktop 要 Computer Use fixture。resolver/bubble/committer code path 全在位（upload.ts / SkillInstallConfirmBubble / skill-install-committer）；留到 upload UI 落地后一次跑 9.4/9.7/9.8/9.12
- [ ] 9.5 Desktop 用户说 "同步一下 Claude 里关于 review 的 skill" → `sync_from_claude_code` 返全量候选 → LLM 按 filter 挑一个 → 单独触发一个 confirm → 装 — **observation 2026-04-20**：需 `~/.claude/skills/` fixture + 可靠 LLM 路由命中 `sync_from_claude_code`，后者按 9.6 不稳定。resolver 契约在 `claude-code.ts`（50 候选上限 + `sync-too-many-candidates`）；handler 返 `{ kind: 'sync-candidates', source: 'claude-code', candidates }`。留 T2.2+
- [ ] 9.6 Web 用户调 `sync_from_claude_code` → tool 返 `not-supported-in-web` → LLM 跟用户解释 + 引导切桌面端 / 上传 — **observation 2026-04-20**：Codex 真实浏览器重放 prompt，boss 层 LLM 未稳定命中此 tool，走了澄清/转派。resolver 契约确定（`claude-code.ts` 在 runtime 非 desktop 时必返 `not-supported-in-web`）；残留是 LLM 路由稳定性，非实现缺失
- [ ] 9.7 **T2.1 deferred 11.8 (path-traversal)**：构造一个 upload zip，包含 `scripts/../../../etc/passwd` 路径项 → preview 阶段允许 LLM 看到，但 confirm → `installSkill` 守卫拒；写 observation — **observation 2026-04-20**：同 9.4 class，需 upload fixture。`skill-loader.ts` `installSkill` tier-3 守卫拒 `..` / 绝对路径 / 非 `scripts|references|assets` 前缀（disk write 前），rollback 路径在 write-through wrapper
- [ ] 9.8 **T2.1 deferred 11.7 (slug collision)**：先装一个 `do-research` → 再装另一个不同 source 的同 slug → 第二次 preview 可出，但 confirm 拒 + 返回错误给 chat — **observation 2026-04-20**：mechanical 两步安装，未脚本化 live。`skill-loader.ts` installSkill 按 `(companyId, null, slug)` 或 `(companyId, employeeId, slug)` 查，mismatched `source_ref` 抛 `SkillInstallError` `slug-collision`
- [ ] 9.9 **T2.1 deferred 11.9 (cross-scope override)**：先 company scope 装 `email-triage` → 再给 Alice 装 `email-triage` → 给 Alice 出 prompt 时只看到 Alice 那份 — **observation 2026-04-20**：两步 install 机械验证，未脚本化 live。`skill-loader.ts` `listSkillsForEmployee` 合并 employee 覆盖 company；DB 两条 partial UNIQUE（`WHERE employee_id IS NULL` / `IS NOT NULL`）让同 slug 双 bucket 共存
- [ ] 9.10 **T2.1 deferred 11.3 (desktop migration)**：Desktop 环境里先塞一个老 `config_json.runtimeSkill` 的员工 → 首次触发 `installSkill` → 观察 migrateRuntimeSkills 同时跑完、marker 写入、新 runtimeSkill 在 employee config 被 strip — **observation 2026-04-20**：需 fixture DB 或往 Application Support 手写 SQL。`skills-bootstrap.ts` `migrateRuntimeSkills` 扫全员 → synthesize employee-scope SKILL.md + `source_kind='synthesized'` 行 + strip 旧字段 + 写 `settings.skills_migration_v1_done`；`onVaultReadyForSkills` 首次 vault activation 触发（web/tauri/tauri-lite 三 runtime 共用）
- [ ] 9.11 Cancel / timeout 流：触发一次 `install_skill_from_git` 后 cancel → staging 清空、skills 无新行；再触发一次后等 30 分钟过 TTL 模拟 → confirm 返 `staging-expired` — **observation 2026-04-20**：cancel 可达，但 web `respondToInteraction()` 不把 committer outcome 回写 chat（静态返 `"Skill installed." / "Skill install cancelled."`），TTL 也没可观测结果面。后端 `skill-install-committer.ts` 返 `{ status: 'staging-expired' }`；`skill-staging.ts` GC 过期清理。留 T2.2+（扩 interaction follow-up pipe 带 outcome）
- [ ] 9.12 宽口 pattern 红标：构造一个 SKILL.md `allowedTools: ['bash:*', 'network:read']` → preview bubble `bash:*` 红标，`network:read` 正常；截图入 live-verify record — **observation 2026-04-20**：同 9.4 class，需 upload fixture。`SkillInstallConfirmBubble` 读 interaction `severity: 'high'` flag 渲染宽口 token；`isWideScopePattern`（`/^(bash|network|fs|exec)(:|\*)/iu`）在 `skill-install-tools.ts` driving severity
...
- [x] 10.2 Tasks 一致性：本 tasks.md `[x]` 项全部落到代码 + build + typecheck 均 green。Live verify 本轮收口：9.1 / 9.2 / 9.3 真实 runtime PASS；9.4–9.12（9 条）按 observation 方式归档，`[ ]` 保留 + 每条注上"为什么没跑 live + code 路径证据 + 什么时候补"。观察到的共通 gap：(i) 没 upload UI affordance（9.4 / 9.7 / 9.8 / 9.12）；(ii) web interaction follow-up pipe 没把 committer outcome 传回 chat（9.11 `staging-expired` 无观测面）；(iii) boss-layer LLM 路由 `sync_from_claude_code` 不稳定（9.6 残留风险是路由层，不是 resolver 契约）。三条都留作 T2.2+ followup，不阻断 archive
