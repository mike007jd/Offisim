...
- [ ] 8.4 `apps/web build` 产物 bundle size — **超出 30 KB gzip 目标**：实测 `skill-install-env-*.js` lazy chunk = 186.97 KB raw / 54.17 KB gzip。主因 fflate 在 core 拉一份、ui-office 拉一份，两份合进同一 code-split chunk。Lazy 化意味着只在 skill install 触发时才 load，不影响 main bundle 首屏。留作 T2.2+ followup：fflate 统一走 ui-office 共享 chunk 或同包 hoist 到一处。Archive 前应记入 live-verify/debt
