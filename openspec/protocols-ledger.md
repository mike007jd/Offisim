...
| 2 | **MCP Transport (TypeScript SDK)** | `client ✓; resources/prompts surfacing + Streamable HTTP + OAuth pending`。Code-verified 2026-05-17: `packages/core/src/mcp/mcp-tool-executor.ts` owns stateful tool/resource/prompt catalogs and calls, `packages/core/src/mcp/types.ts` supports current `stdio`/`sse`, and execution still flows through `AuditingToolExecutor` + `ToolPermissionEngine`. UI/registry may record legacy `sse`; desktop remote connect remains scoped by `openspec/specs/mcp-transport-decision.md`. | Context7 2026-05-02：`StreamableHTTPClientTransport` 是现代远程 client transport；client-side `SSEClientTransport` 仍用于 legacy SSE server fallback；server-side SSE v2 已移除/弃用 | ⚠️ | Shipped client stays stdio/SSE for now; do not advertise resources/prompts UI surfacing, Streamable HTTP, or OAuth until auth/header storage, reconnect/health, legacy saved-entry compatibility, and release evidence exist |
...
| 8 | **Anthropic Claude Agent SDK** | `@anthropic-ai/claude-agent-sdk 0.2.132` + `@anthropic-ai/sdk ^0.95.0`；Offisim 通过自有 `ClaudeAgentSdkAdapter` / trusted-host bridge 把 SDK 用作 `offisim-core` model transport/provider adapter，不作为普通 SDK lane；MiniMax Intl 与 Z.AI 有 Claude Agent SDK model transport evidence，详见 `openspec/provider-lane-matrix.md`；rich agent 能力属于 `RuntimeEngineCapabilityProfile` / main-harness control-plane，当前 Claude engine 是 text-only preview profile | Anthropic 当前主推 `@anthropic-ai/claude-agent-sdk`；TypeScript SDK 自带 Claude Code native binary，可通过 `ANTHROPIC_BASE_URL` / `ANTHROPIC_AUTH_TOKEN` 走 LLM gateway；第三方产品需用 API key / cloud auth，不应复用 claude.ai 登录/订阅配额 | ⚠️ | 未验证 model transport 不暴露 file/shell/memory/todo/skill/MCP/builtin tools；当前工具工作走默认 Offisim harness / gateway evidence path；full-agent / gateway-bridged Claude employee profile 必须先补 denied-path、cancellation、checkpoint/resume、telemetry、rollback、release evidence |
| 9 | **OpenAI Agents SDK** | `@openai/agents 0.9.1` + `openai ^6.36.0` + `OpenAiAgentsSdkAdapter` + trusted-runtime transport override 已落；native OpenAI / OpenAI-compatible rows仍以 `openspec/provider-lane-matrix.md` 为证据源；Codex local-auth product 当前只是 model transport evidence；transport binding 不能自升为 employee-agent / main-harness driver / replacement | npm `@openai/agents` latest 为 0.9.1；OpenAI 2026-04 Agents SDK harness / sandbox 更新主要是能力口径和 Python `openai-agents>=0.14.0` 路径，不代表本 repo 使用的 TypeScript 包已到 2.0 | ⚠️ | 先补 OpenAI native smoke / load / edge evidence，再逐家验证 OpenRouter / Kimi / Z.AI / Gemini / DeepSeek / Qwen compat variant；未有实证前产品不宣称 OpenAI Agents SDK full-agent runtime verified，更不能从 model transport evidence 推导 full-agent employee support |
