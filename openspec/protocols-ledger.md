...
| 6 | **SKILL.md 开放标准** | **未接**（Offisim skill 当前作为"能力包"嵌在 `employee.config_json.capabilityIndex`，不是独立一等公民，不是 SKILL.md 格式） | Anthropic SKILL.md spec（2025-12 开放标准 / `anthropics/skills` 120k stars / Hermes / Claude Code / Cursor / Codex / Copilot / Windsurf / Gemini 等 19 agent 互通 / 350k+ packages） | ❌ | **T2.1 直接接**：全局 + 员工专属两层 skill 都用 SKILL.md 格式，目录结构 `{skill-slug}/SKILL.md + scripts/ + references/ + assets/`；progressive disclosure 3 层（启动载 name+desc / 激活载 body / 按需载 scripts） |
