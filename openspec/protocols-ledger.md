...
| 5 | **LangGraph / checkpoint** | `@langchain/langgraph ^1.2.1` + `langgraph-checkpoint-sqlite ^1.0.1` + `langgraph-checkpoint ^1.0.0`；**web 侧自维护 fork** `apps/web/src/lib/tauri-checkpoint.ts`（手抄 upstream `SqliteSaver` 用 Tauri async SQL） | LangGraph JS 主 API 当前形态 | ⚠️ | **T1.3** 显式标注 fork 差异 + 建立定期对比 upstream 机制；风险是未来 upstream 加 filter / migration 时本地悄悄落后 |
| 6 | **SKILL.md 开放标准** | **已接**（T2.1 `add-skills-foundation-two-tier-schema`）：`packages/core/src/skills/skill-md.ts` parser + serializer 只认 Anthropic 标准字段（`name` + `description` 必填，可选 `allowedTools` / `license` / `version`），禁 `offisim.*` 私有命名空间；`SkillLoader` 3-tier API（`listSkillsForEmployee` / `loadSkillBody` / `loadSkillAsset`）；两层 schema `skills` 表（company / employee scope）；vault 磁盘布局 `companies/{cid}/[employees/{slug}/]skills/{slug}/SKILL.md` | Anthropic SKILL.md spec（2025-12 开放标准 / `anthropics/skills` 120k stars / Hermes / Claude Code / Cursor / Codex / Copilot / Windsurf / Gemini 等 19 agent 互通 / 350k+ packages） | ✅ | 观察上游是否加 `version` 等官方字段；目前 `allowedTools` 作 Anthropic 允许的扩展字段 |
