...
Empty, error, and default states in touched workspaces provide a concise title, a user-level reason or context, one primary next action, and an optional secondary action, and SHALL NOT surface bare technical errors as their only explanation. SOP starts with a clear template/create/import workflow; Market degrades gracefully when platform data is unavailable; Activity explains what event families appear; Settings keeps the save area and form width readable and explains disabled Save; Studio and Employee Creator provide clear empty states and non-obscuring bottom actions.
