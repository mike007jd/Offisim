# workspace-thread-architecture Specification

## Purpose

Project ↔ thread is one-to-many in the data layer; the right rail nests Project → Thread → Chat. High-traffic affordances (Mode selector, Notification center, Dashboard, Install entry, Kanban) are relocated out of the application header and right-rail Tasks shell so they no longer fight for the same real estate, and the Tasks tab + Kanban surface are gated on actual content rather than always-rendered. The capability is the discoverability anchor for "where does X live in the workspace shell".
