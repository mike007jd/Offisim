# agent-mediated-skill-install Specification

## Purpose

Make "install a skill" an agent-native chat action rather than a Marketplace-only UI flow. Every employee (internal and A2A external alike) carries four skill-install tools — `install_skill_from_git`, `install_skill_from_upload`, `sync_from_claude_code`, `sync_from_codex` — that stage a SKILL.md candidate plus its optional `scripts/` / `references/` / `assets/` siblings, surface a user-facing `skill_install_confirm` interaction with security-relevant metadata (parsed SKILL.md, `allowedTools` with wide-scope patterns flagged, source origin, resolved scope, staged asset paths), and only mutate the vault + `skills` table after the user explicitly confirms. Scope resolution is LLM-driven via tool parameters — there is no separate UI scope selector. All writes funnel through a single `SkillLoader.installSkill` entry point that reuses the `skills-foundation` tier-3 guards (path traversal / absolute paths / asset subtree whitelist) and its partial `UNIQUE` slug rules, so agent-install and Marketplace-install share the same safety envelope. Does NOT cover fork / self-create / peer-transfer / skill body editing UI / registry publishing — those remain T2.3+ future scope.
## Requirements
...
- **THEN** `SkillInstallCommitter` executes the commit phase, writes `companies/{cid}/employees/{slug}/skills/{slug}/SKILL.md`, inserts the `skills` row with `source_kind='self-authored'`, and the chat surface renders `Skill {slug} created from scratch.` per the unified `skillInstallOutcomeLabel` SSOT

#### Scenario: Cancel 'create' discards staging

- **WHEN** a `skill_install_confirm` interaction with `action='create'` is cancelled
- **THEN** `skillStagingManager` discards the staging entry, no vault file is written, and the chat surface renders `Skill action cancelled.` per the unified `skillInstallOutcomeLabel` SSOT
