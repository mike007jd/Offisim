# personnel-runtime-engine-binding Specification

## Purpose

Defines the contract for the Personnel `Runtime` tab and the Settings → "Default employee runtime" surface that wire `EmployeeRuntimeBinding` and `RuntimePolicyConfig.employeeRuntimeDefault` to user-visible controls. Internal employees see a four-way picker (`Inherit company default` / `Provider gateway` / `Claude engine` / `Codex engine`) backed by `formData.runtimeBinding` and the existing `useEmployeeEditor` save path. External employees see a read-only lock card. Engine choices are gated on `availableEngineAdapters` from `OffisimRuntimeContext`; engine mode resolves to a "Preview · limited tool telemetry" disclosure regardless of source. The shared `<RuntimeBindingControl>` primitive renders both Personnel and Settings surfaces with a `scope` prop.
