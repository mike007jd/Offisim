# scene-activity-display Specification

## Purpose

统一 "employees active" 计数显示的权威来源。StatusBar footer `{active}/{total} employees` 与 3D overlay `{activeCount} employees active` / `{blockedCount} employees blocked` 都从单一共享 hook `useActiveEmployeeCount()` 读取 `{ active, total, blocked }`——任一 UI surface 不得再自行维护 `Map<employeeId, EmployeeState>` 做计数或重复订阅 `employee.state.*`。active 判定集合显式锁死为 `assigned / thinking / searching / executing / meeting / reporting / waiting`，`idle / success / paused / blocked / failed` 一律不计入 active（`blocked / failed` 由独立的 `blocked` 计数单独报告）。run-start 与 company-switch 两类 reset 路径对两个消费者对称生效；`employee.state.*` / `employee.created` / `employee.deleted` 事件在同一 React render cycle 内同步到两处显示。
