# marketplace-official-seed Specification

## Purpose

Guarantee that a freshly started Offisim platform always carries one Offisim-authored Market listing per AssetKind so the Market workspace has previewable, installable content from the first boot. Covers the boot-time seeder (idempotent on `creators.handle = 'offisim'`), the seed payload sources (repo-sourced for `employee` / `company_template` / `prefab`, hand-authored for `skill` / `sop` / `office_layout`), the in-memory `.offisimpkg` artifact cache (rebuilt on every boot so install survives restarts), the platform-served `/v1/install/artifacts/:versionId` route, the 7-option `KIND_FILTERS` UI surface, and the install/preview gating around `INSTALLABLE_KINDS`. Does NOT cover publish-time `offisim/` prefix enforcement (deferred follow-up) or extending `INSTALLABLE_KINDS` to additional kinds (each new installer is its own change).
