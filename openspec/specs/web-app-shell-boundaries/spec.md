# web-app-shell-boundaries Specification

## Purpose

`apps/web/src/App.tsx` 的职责边界规范 —— App.tsx 作为 web SPA shell composition root 必须保持 "thin"：只做 orchestrator hook 组合 + `useMemo` 派生 + 3 个 app-shell 组件挂载。company lifecycle / company bootstrap / office-scoped bindings / keyboard shortcuts / overlay state 五个职责全部由对应 hook 拥有，AppLayout 9 slot JSX + 6 overlay 分支 + 5 global dialog 分支分别由 `AppMainShell` / `AppOverlayHost` / `AppGlobalDialogs` 三个 render-only 组件承担。`App` 命名导出签名 `(props: { onCompanySwitch: (id: string | null) => void }) => JSX.Element` 不变，`apps/web/src/main.tsx` 的 lazy import 路径保持 `./App.js`。本规范约束**结构边界**，不改 observable behavior：`unified-shell-routing` + `workspace-state-management` 已有的 scenario 全部继续 pass。
