# panel-and-dialog-sizing Specification
...
#### Scenario: Touched dialog imports the constants
- **WHEN** auditing `packages/ui-office/src/components/employees/EmployeeEditorDialog.tsx`
- **THEN** the file SHALL import `DIALOG_SIZING_CLASS`, `DIALOG_TABS_ROOT_CLASS`, and `DIALOG_TABS_CONTENT_CLASS` from `@offisim/ui-core`
- **AND** the file SHALL apply them to `DialogContent`, `Tabs.Root`, and every `TabsContent` respectively
