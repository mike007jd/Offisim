# shell-interaction-baseline Specification

## Purpose

The application shell — Header peer-workspace nav, Settings tab nav, right rail collapse handle, Notification badge, chat input list menus — guarantees a small set of primitive interaction invariants that the rest of the product relies on. A single primary left-click navigates one surface tier in one commit cycle. The right rail collapse handle is reachable any time the rail is expanded. Visual chrome (notification badge) does not depend on ancestor `overflow: visible` to render. Keyboard navigation in vertically scrollable list menus keeps the active row in view. These invariants apply across viewport tiers and themes; implementations must address the actual root cause rather than masking symptoms with debounces, fallbacks, or z-index escalations.
