...
Header SHALL render peer workspace navigation separately from Office-scoped tools while continuing to use the single `AppLayout` shell for all workspaces. Peer workspace controls SHALL change `activeWorkspace`; Office tool controls SHALL open Office overlays or Office-scoped UI without becoming peer workspace routes. There SHALL be exactly six peer workspaces: Office, SOPs, Market, Personnel, Activity, and Settings.

#### Scenario: Office header groups navigation by scope
- **WHEN** `activeWorkspace` is `'office'`
- **THEN** Header renders peer workspace navigation for Office, SOPs, Market, Personnel, Activity, and Settings
...
- **WHEN** `activeWorkspace` is `'sops'`, `'market'`, `'personnel'`, `'activity-log'`, or `'settings'`
...
At constrained widths, Header SHALL preserve access to peer workspace navigation and active workspace identity before exposing secondary Office tools. Office tools MAY collapse into a menu or overflow control. Peer workspace overflow MAY collapse Personnel and Activity behind a menu before Settings or Market.
