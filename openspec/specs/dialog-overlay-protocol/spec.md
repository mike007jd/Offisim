# dialog-overlay-protocol

## Purpose

Every modal dialog and full-screen overlay shares a single close / focus / shortcut protocol. Close button, Escape, Cancel, Back, and completion actions all route through the same overlay-state owner (or explicitly block close with a visible dirty/validation reason). The topmost overlay owns Escape and Office keyboard shortcuts do not leak through to underlying workspace state. Modals trap tab focus while open and restore focus to the invoking control on close. Backdrop-click behavior is declared explicitly per surface; dirty/destructive/wizard surfaces must not discard progress on backdrop click.
## Requirements
