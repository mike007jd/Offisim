...
try {
  ...
  for (let i = 0; i < allInputs.length; i++) {
    ...
    console.log(`  Input ${i + 1}: type=${inputType}, placeholder="${inputPlaceholder}", value="${inputValue}", visible=${isVisible}`);
  }
  ...
  for (let i = 0; i < allButtons.length; i++) {
    ...
    if (isVisible) {
      ...
      console.log(`  按钮 ${i + 1}: text="${text}", disabled=${isDisabled !== null}, class=${btnClass ? btnClass.substring(0, 100) : 'none'}`);
    }
  }
  
  console.log('步骤 7: 查找并点击创建按钮');
  const createButton = page.locator('button:has-text("Create"), button:has-text("创建"), button:has-text("Confirm"), button:has-text("确认")').first();
  ...
  if (createBtnVisible) {
    ...
    const modalStillVisible = await page.locator('[role="dialog"]').first().isVisible().catch(() => false);
  ...
  if (sidebarVisible) {
    const sidebarItems = await page.locator('aside li, nav li, [role="navigation"] a, [role="navigation"] button').all();
